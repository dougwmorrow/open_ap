...
from extract.connectorx_oracle_extractor import extract_oracle_connectorx, extract_oracle_connectorx_windowed
from extract.connectorx_sqlserver_extractor import extract_sqlserver_connectorx, extract_sqlserver_connectorx_windowed
from extract.udm_connectorx_extractor import read_stage_table, read_stage_table_windowed, table_exists
...
def _extract_source_windowed(table_config: TableConfig, start_date, end_date, output_dir) -> tuple:
    """Extract windowed source data for reconciliation.

    Uses ConnectorX windowed extractors which filter on
    table_config.source_aggregate_column_name.
    """
    if table_config.is_oracle:
        return extract_oracle_connectorx_windowed(
            table_config, output_dir, start_date, end_date,
...
def reconcile_table(
    ...
    try:
        ...
        internal_cols = {
            ...
            "_cdc_is_current", "_cdc_batch_id", "UdmModifiedBy",
        }
        source_cols = [
...
def reconcile_table_windowed(
    ...
    Extracts source rows within [start_date, end_date) and compares them
    against current Stage CDC rows in the same window. This is the safety
    net for windowed CDC limitations (P1-4 phantom deletes, V-7
    cross-midnight overlap, P3-9 late-arriving data).

    Args:
        table_config: Table configuration.
        date_column: Business date column for window filtering
            (should match table_config.source_aggregate_column_name).
    ...
    try:
        ...
        # --- Extract source data for the window ---
        logger.info(
            "C-7: Extracting source data for %s [%s, %s)",
            table_config.source_full_table_name, start_date, end_date,
        )
        df_source, _ = _extract_source_windowed(
            table_config, start_date, end_date, output_dir,
        )
        result.source_rows = len(df_source)

        if len(df_source) == 0:
            logger.info("C-7: Source extraction returned 0 rows for window")

        # --- Read windowed Stage current rows ---
        logger.info("C-7: Reading Stage table %s [%s, %s)",
                     stage_table, start_date, end_date)
        df_stage = read_stage_table_windowed(
            stage_table, date_column, start_date, end_date,
        )
        result.stage_rows = len(df_stage)

        if len(df_source) == 0 and len(df_stage) == 0:
            logger.info("C-7: Both source and Stage empty in window — clean")
            result.matched_rows = 0
            return result

        if len(df_stage) == 0:
            result.source_only_rows = len(df_source)
            result.errors.append("Stage table has 0 current rows in window")
            return result

        # Check for duplicate PKs in Stage (proxy for data issues)
        ...
        if sample_size and len(df_source_cmp) > sample_size:
            df_source_cmp = df_source_cmp.sample(n=sample_size, seed=42)
            logger.info("C-7: Sampled %d rows from source", sample_size)
        ...
        if result.is_clean:
            logger.info(
                "C-7: Windowed reconciliation PASSED for %s.%s: "
                "source=%d, stage=%d, matched=%d, 0 mismatches",
                table_config.source_name, table_config.source_object_name,
                result.source_rows, result.stage_rows, result.matched_rows,
            )
        else:
            logger.warning(
                "C-7: Windowed reconciliation FAILED for %s.%s: "
                "source=%d, stage=%d, matched=%d, mismatched=%d, "
                "source_only=%d, stage_only=%d, mismatched_columns=%s",
                table_config.source_name, table_config.source_object_name,
                result.source_rows, result.stage_rows,
                result.matched_rows, result.mismatched_rows,
                result.source_only_rows, result.stage_only_rows,
                result.mismatched_columns,
            )
    ...
    # OBS-6: Persist result for historical trending
    _persist_reconciliation_result(
        check_type="WINDOWED_RECONCILIATION",
        table_name=result.table_name,
        source_name=result.source_name,
        is_clean=result.is_clean,
        source_rows=result.source_rows,
        ...
        metadata={
            ...
            "mismatched_columns": result.mismatched_columns,
        } if result.mismatched_columns else {
            "start_date": str(start_date),
            "end_date": str(end_date),
        },
        errors=result.errors or None,
