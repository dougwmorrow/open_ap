...
- tests/integration/ -- Tier 3 integration test scaffold per docs/migration/phase1/05_tests.md section 1.3 + section 6.2 (testcontainers-python + mcr.microsoft.com/mssql/server:2022-CU14-ubuntu-22.04 image; module-level skip on dev workstations without Docker). 6 files: __init__.py + conftest.py (canonical fixture set: _docker_available session-probe / mssql_container session-scope / mssql_connection function-scope / mssql_cursor function-scope / test_db_transaction function-scope BEGIN-ROLLBACK / canonical_schema_loaded session-scope + docker_skip_marker() factory) + 3 example test files (test_idempotency_ledger_concurrency / test_parquet_write_verify_replay_chain / test_extraction_state_machine = 11 tests total; all module-level skipped at scaffold-landing pending B-115 follow-up Docker-fixture activation). Round 6 follow-up build 2026-05-14 on branch round-6-post-merge-tracking; closes B-115 scaffold milestone (B-115 itself stays open for the next iteration to actually wire schema.sql + seed_data.sql + activate the skip-decorator). Companion: tests/fixtures/udm_test_fixtures/ shared-fixture home per section 1.3 (currently empty __init__.py; schema.sql + seed_data.sql land at B-115 follow-up).
