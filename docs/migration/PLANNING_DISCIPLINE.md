...
| **PS-6 COHORT** | `udm-context-loader` (skill; per-spawn CCL distillation; emits brief with PASS-THROUGH-VERBATIM Do-NOT + Pitfall #9.x + D-N/R-N + skill-inheritance per scope), `MULTI_AGENT_GUIDE.md § Canonical Context Load` (D62 doc-discipline fallback when skill unavailable; sub-agents apply at spawn time per CLAUDE.md hard rule 13 inheritance contract), `udm-step-10-verifier` (skill), `udm-gap-check` (skill), `udm-progress-logger` (skill) | + cascade-specific skills per cohort scope; `udm-post-build-verify` (skill) if code | Parent invokes `udm-context-loader` PER sub-agent spawn (~80% CCL substrate cost reduction across a 5-agent cohort per MARKDOWN_REFACTOR_PLAN.md §4.5 + §15.4 empirical baseline); CCL discipline at sub-agent spawn (binding per CLAUDE.md hard rule 13; sub-agent's first Read targets Stage 3 spec doc cited in brief, since Stage 1+2 substrate already distilled in brief); verifier after build; gap-check after; logger throughout; post-build-verify for pytest re-run after parallel cohort. **Note**: newly-authored `udm-context-loader` skill (B-275 ⚫ CLOSED 2026-05-17) operationalizes this discipline as a single-Skill invocation. |
...
- **`udm-post-edit-verification`** (added 2026-05-15 per user-direction + CLAUDE.md hard rule 14) — MANDATORY 3-step cascade (TEST + GAP ANALYSIS + REVIEW) after ANY substantive edit. Supersedes prior minimum-viable-set pragmatic exemption pattern that allowed `udm-gap-check` to be deferred across consecutive same-session commits (3-event evidence base from D.3 + cleanup + D.4). Anti-triggers (typo / strikethrough flip / cosmetic) skip with explicit justification per commit-message output contract.
