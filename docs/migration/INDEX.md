...
- `_archive/` — **"Where do old/superseded artifacts go?"** Currently empty (subdirectory not yet created). Will materialize when Phase D.1 archive sweep runs per `MARKDOWN_REFACTOR_PLAN.md` D.1. Until then, all docs remain in `docs/migration/` root or `phase1/` / `phase2/`.
