...
### ~~P-17 (🟡 Open):~~ Round 5 § 5.6 strict `<=` needs ULP-tolerance note for percentile monotonicity — ⚫ **CLOSED 2026-05-15 (superseded by B-271 production fix)**
...
- **Closure mechanism (2026-05-15)**: P-17's framing was INCOMPLETE — empirical evidence on 2026-05-15 showed `statistics.quantiles()` produces non-monotonic cuts that differ by **multi-ULP** (~4 ULPs at value 29.7, NOT just bit-equal-within-1-ULP). The actual fix required production code change (monotonicity-clip in `cdc/lateness_profiler.py::_compute_percentile()`) — see B-271 for full closure detail. P-17's spec-wording polish item became moot once the production-side guarantee was in place. **Lessons captured**: (a) when a P-N is opened on the basis of "behavior is correct; only spec wording drifts" — verify the empirical assumption by exhaustive-property-testing BEFORE accepting the polish-only framing (P-17's "within 1 ULP" claim was an unverified assumption); (b) Hypothesis property tests are the right verification mechanism — same lesson as B-262 (NFC-before-Categorical-cast also surfaced via Hypothesis on 2026-05-14). Pitfall #9 sub-class candidate: "polish-item-framing-precision-error" (P-N opened with too-narrow scope; later proves to need B-N treatment) — 2-event evidence base (P-17 + a hypothetical earlier instance if surfaced).
...
- **P-24** (⚫ CLOSED 2026-05-19; LOW): ~~**Document LIFTED-Do-NOT-rule format in CLAUDE.md Do-NOT-rules section header**~~ — **⚫ CLOSED 2026-05-19** via this commit: CLAUDE.md L308+ Do-NOT-rules section header extended with **Rule format** paragraph documenting canonical LIFTED-rule format (`~~Do NOT <body>~~ — **LIFTED YYYY-MM-DD** via <B-N closure citation>. <Closure mechanism summary>. <Operational guidance>. <Original empirical anchor preserved>`). First-instance precedent cite to B-545 LIFTED at L350 + reference to gap-check reviewer `ac5c9ea53cc34bce3` G5.2 finding (stale forward-reference within LIFTED body class). Future Do-NOT rules that get superseded should follow this pattern. **Original body**: **Document LIFTED-Do-NOT-rule format in CLAUDE.md Do-NOT-rules section header** — first instance landed at commit `00039a1` 2026-05-19 (B-553/B-554 closures lifted the production-safety rule for `tools/validate_parquet_vs_stage.py --apply` against StripSuffix=1 + NULL-PK tables). Format chosen: `~~Do NOT ...~~` strikethrough body + **LIFTED YYYY-MM-DD** annotation + B-N closure citations + empirical-anchor preservation. Documenting the pattern at the Do-NOT-rules section header would prevent future format-inconsistency drift when subsequent rules need lifting. Cosmetic-only; doesn't change discipline. Closure target: opportunistic at next round close-out OR next Do-NOT rule lift (whichever first). Source: cross-cohort reviewer Agent `a8130cf417bb5692a` 2026-05-19 Scope 5 format-precedent finding.
