...
| **CommitMsgCheck** | `tools/check_commit_msg.py` | Abstract base class for commit-msg orchestrator checks per B-459 closure 2026-05-18. Subclass contract: declare class attributes `name: str` (unique identifier; used as audit-row JSON findings dict key) + `severity: Literal["WARN","BLOCK"]` + `requires_backlog_diff: bool`; implement `scan(commit_msg, ctx: OrchestrationContext) -> CheckResult` (per B-467 closure 2026-05-18; was `scan(commit_msg, staged_diffs)` pre-B-467). Orchestrator contract: iterate `CHECKS` registry + pass commit_msg + OrchestrationContext (carrying staged_diffs + classification, batched once per main() entry) to each subclass instance; collect findings under `check.name` key in audit-row findings dict; only `severity=BLOCK` contributes to final exit code. Replaces 4-way copy-paste of `_strip_code_blocks` + WARN-only + try/except + audit-row-extension patterns previously duplicated across exemption-phrase + cascade-evidence + B-449 + B-451 checks. |
...
| **_build_orchestration_context** | `tools/check_commit_msg.py` | Helper per B-467 closure 2026-05-18 — builds canonical OrchestrationContext once per main() invocation. Signature: `(checks: list[CommitMsgCheck]) -> OrchestrationContext`. Calls `classify_commit()` only when CascadeEvidenceCheck instance is in checks list (skip optimization for synthetic/test contexts without cascade check); graceful exception swallow returns `classification=None`. Replaces pre-B-467 `_collect_staged_diffs(checks)` helper which only batched staged_diffs (B-467 extended to also batch classification). |
