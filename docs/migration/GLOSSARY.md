...
| `.githooks/commit-msg` (Mechanism C-1 companion per B-307 split 2026-05-16) | `.githooks/commit-msg` <- introduced 2026-05-16 via B-307 split closure (Q3 critical coverage gap from B-301 proactive reviewer agentId `ade062dd2b158d7a2`); Python script (117 lines per actual wc -l) receiving COMMIT_EDITMSG path as $1 (commit-msg hook contract); reliable for ALL commit modes including `git commit -m` direct-message commits which the pre-commit hook missed (COMMIT_EDITMSG not populated at pre-commit time); strips git-comment lines before phrase detection; 12-phrase trigger list per SKILL.md L29-46 → BLOCKS commit with reviewer-spawn instruction. Same `git config core.hooksPath .githooks` installation activates both pre-commit + commit-msg hooks atomically. Tier 0 test at `tests/tier0/test_commit_msg_hook.py` (11/11 pass). |
| `.githooks/pre-commit` (Mechanism C-1 per CLAUDE.md hard rule 14 step 7 + B-301 closure; refactored per B-307 split 2026-05-16) | `.githooks/pre-commit` <- introduced 2026-05-16 via B-301 trigger-met closure (Pitfall #9.o instances 8 + 9 evidence); Python script (177 lines) mechanically enforcing 2 deterministic checks at commit-time per harness-automated structural fix; (a) `tools/query_blindspots.py --severity p0,p1 --live` scan on each staged file → BLOCKS on p0 per D74 exit-code-2; (b) commit-message exemption-phrase detection with 12-phrase trigger list per `udm-exemption-verifier` SKILL.md L29-46 + B-303 extensions → BLOCKS with reviewer-spawn instruction. Producer cannot bypass mechanically without `--no-verify` (self-flagging). **Installation (one-time per clone)**: `git config core.hooksPath .githooks`. **Acknowledged limitations**: (i) hook cannot invoke Claude API directly; reviewer invocation remains producer-responsibility in Claude Code session; (ii) chicken-and-egg on own authoring commit (instance-N+1 candidate; this very commit produced 3 known-false-positive p0 matches on hook + test + SKILL.md trigger-phrase data); (iii) `git commit -m` direct-message commits BYPASS exemption-phrase check because COMMIT_EDITMSG is not populated at pre-commit hook execution time — tracked as B-307 for move to commit-msg hook (where COMMIT_EDITMSG is reliably populated for all commit modes). 9-event empirical evidence anchor (Pitfall #9.o instances 5-9 all proved producer-judgment-based discipline empirically vulnerable; this is the structural-fix-via-harness-automation completing Mechanism A + Mechanism B). Tier 0 test at `tests/tier0/test_pre_commit_hook.py` (11/11 pass). |
| `udm-exemption-verifier` (Mechanism B per CLAUDE.md hard rule 14 + B-296 closure) | `.claude/skills/udm-exemption-verifier/SKILL.md` <- introduced 2026-05-16 via B-296 trigger-met closure (Pitfall #9.o instance 7 commit `01d32c0`); fires when commit message contains hard-rule-14 cascade exemption-claim phrasing ("Layer N+1 termination" / "recursive-exemption" / "verbatim implementation" / "100% overlap" / "REVIEW: SKIPPED" / etc.); 5-min budget cap; binary VALID/INVALID-with-specific-files verdict; INVALID triggers spawn of `udm-gap-check` per D56 second-pass + BLOCKS commit until verdict flips to VALID via remediation. **Skill exempt from its own recursion** (single-purpose audit; binary output; no exemption-claim-on-exemption-verifier-output recursion). 7-event empirical evidence anchor (Pitfall #9.o instances 5/6/7 all proved Mechanism A structural-fix-via-documentation insufficient within ≤24 hours). **Structural-fix-via-agent-invocation** — shifts verification from producer self-judgment to independent third-party agent invocation. Integration: `udm-post-edit-verification` SKILL.md Step 2.5; CLAUDE.md hard rule 14 step 6. |
| `udm-next-step-cascade` (user-triggered 2-step proceed+gap-check cascade) | `.claude/skills/udm-next-step-cascade/SKILL.md` <- introduced 2026-05-14 per user-direction "We shouldnt always proceed with next steps"; explicit trigger constraint -- fires ONLY on imperative phrases "Proceed with next steps" / "Move forward" / "Continue with next steps" / similar; NEVER as default behavior between turns; closes the "agent always proceeds with next steps autopilot" anti-pattern. 2-step procedure: (1) execute recommended next step from prior turn + udm-progress-logger + Step 10 + commit; (2) gap-check via udm-step-10-verifier + parent-agent reflection on Pitfall #9.j-9.n surfaces. EXTENDED 2026-05-15 with sub-agent skill inheritance contract section per CLAUDE.md hard rule 13 + `PLANNING_DISCIPLINE.md` §3 (when cascade runs within active planning session AND spawns sub-agents). |
| `udm-session-compactor` (session-state compression skill; Phase 1 manual-trigger) | `.claude/skills/udm-session-compactor/SKILL.md` <- introduced 2026-05-18 per B-492 closure + user-direction "We should add an agent or skill that tracks the context window token usage for a given Claude session... compressing all work done during the session so that when the compression happens, the new context window is able to refer to deeper insights". Authors `docs/migration/_session_snapshots/<YYYY-MM-DD>-<commit-hash-prefix-7>.md` snapshot with 5 canonical sections (Active work context / Completed deliverables / Open runway / Deeper insights / Pointer-back cross-refs). §4 Deeper insights is THE differentiator vs SESSION_RESUME.md — captures architectural decisions + rejected alternatives + cross-cohort patterns + why-not justifications + reviewer-finding context + empirical-evidence accumulations (the WHY that compaction summary loses). Composes with udm-progress-logger (per-completion) + udm-cohort-review (cross-cohort) + udm-round-closeout (round-aggregate) + SESSION_RESUME.md (lightweight pointer; snapshot is the substrate) + udm-gap-check (snapshot completeness verifier). Phase 1 = manual-trigger only; Phase 2 (token-tracking subsystem + automated thresholds) deferred per claude-code-guide research 2026-05-18 finding that hooks cannot access Claude Code token counts (anthropics/claude-code#34340 feature request). 1-event empirical anchor: this session compaction + SESSION_RESUME.md insufficiency surfaced by gap-check reviewer `ab45539c33d1cebd1` G2 finding. Tier 0 test at `tests/tier0/test_skill_session_compactor.py` (8/8 pass). |
| `_session_snapshots/` (per-session-arc deep-insights snapshot directory) | `docs/migration/_session_snapshots/` <- canonical destination for `udm-session-compactor` snapshot artifacts per B-492 closure 2026-05-18. Each file at `<YYYY-MM-DD>-<commit-hash-prefix-7>.md` carries 5 canonical sections (Active work context / Completed deliverables / Open runway / Deeper insights / Pointer-back cross-refs). Distinct from `SESSION_RESUME.md` (lightweight session-pause pointer) — snapshot is the substrate that preserves WHY behind decisions; resume doc is the pointer that points TO the snapshot. Append-only; multi-snapshot-per-day acceptable via `-2` / `-3` suffix for very long sessions. Read at session-resume by next-window agent for deeper context bootstrapping than canonical 5-tracker minimum. |
| `udm-planning-session-startup` (planning-session-startup protocol skill) | `.claude/skills/udm-planning-session-startup/SKILL.md` <- introduced 2026-05-15 per user-direction "What skills should agents use when planning? How do we ensure agents, sub-agents, multi-agent teams use those skills from the start?"; fires on planning-intent trigger phrases ("Let's plan X" / "Plan for Y" / "Design Z" / "Refactor effort for ..." / similar); 5-step procedure (identify PS-N scope / matrix lookup / surface to user / apply throughout / emit §0 provenance); pairs with `udm-next-step-cascade` (forward-motion) + `udm-round-closeout` (round-end) to form canonical lifecycle. Sub-agent inheritance contract section enforces planning-discipline propagation when spawning sub-agents (per CLAUDE.md hard rule 13). |
| `_refactor_log.md` (append-only refactor audit trail) | `docs/migration/_refactor_log.md` <- authored 2026-05-15 per user-direction "Archive EVERYTHING verbatim (belt-and-suspenders)" Option B on refactor-strategy AskUserQuestion. Tracks every markdown-refactor event (TRIM / SPLIT / EXTRACT / RENAME / DELETE / RELOCATE) with: source file:lines → destination → rationale → git commit → equivalence-status → reversibility. Future-trim contract codified in log's §"Future-trim contract" — every PS-2 DOC scope refactor MUST archive verbatim + log + verify destination equivalence. 5 initial entries for D.5 trim (4 retroactive + 1 already-archived gotchas). |
| `_archive/` (verbatim refactor archive subdirectory) | `docs/migration/_archive/` <- created 2026-05-15 per user Option B. Holds verbatim-extracted sections from refactor trims. Each archive file has provenance header citing source / commit / destination cross-ref. Recovery path for non-git-savvy operators (preferred over `git show` for accessibility). Initial 4 archives: CLAUDE_data_flow / CLAUDE_architecture_decisions / CLAUDE_observability / CLAUDE_security_model (all from D.5 trim 2026-05-15 pre-trim CLAUDE.md commit c189432). |
| `INDEX.md` (master routing manifest; CCL Stage 0) | `docs/migration/INDEX.md` <- authored 2026-05-15 per D.2 Phase 1 task 1.3 + MARKDOWN_REFACTOR_PLAN.md §13.2; llms.txt format (H1 + blockquote + sections with linked files + routing-by-intent descriptions per ETH Zurich §3.6 Finding 5); ~126 lines covering 9 categories (CCL Stage 1+2+3 + Canonical registers + Architecture + Process meta-docs + Plans + Phase-specific + Validation/Sidecars). Authored via 3-parallel-agent cohort with CLAUDE.md hard rule 13 sub-agent inheritance contract (4th production application). Read FIRST per CLAUDE.md Read order item 0; cross-refs 54 docs/migration/ files (corrected from initial parent estimate of 57 per udm-gap-check 2026-05-15 G2 finding — Pitfall #9.k arithmetic drift caught + fixed inline pre-commit). |
| `PLANNING_DISCIPLINE.md` (planning-session skill-selection matrix + sub-agent inheritance contract) | `docs/migration/PLANNING_DISCIPLINE.md` <- introduced 2026-05-15; canonical reference for which skills to invoke per planning-scope (9 PS-N codes covering ARCH / DOC / TOOL / SP / RUNBOOK / COHORT / CLOSEOUT / D-N / SELF). Companion to `udm-planning-session-startup` skill + CLAUDE.md hard rule 13. Analogous to `SELF_IMPROVEMENT_DISCIPLINE.md` (D95-D99 framework) and `CHECKS_AND_BALANCES.md` (D55 5-gate framework) as a process-discipline meta-doc. v0.2 grounded in 5 primary sources per `_research/planning-discipline-industry-standards-2026-05-15.md`. |
| `CLAUDE_GOTCHAS.md` (extracted gotcha reference for B-N / E-N / V-N / W-N / OBS-N / SCD2-* / LT-* / DIAG-* / Item-* code-level entries) | `docs/migration/CLAUDE_GOTCHAS.md` <- introduced 2026-05-15 per D.5 Approach A CLAUDE.md trim closure (Q-12 approved per MARKDOWN_REFACTOR_PLAN.md §7.1 task 1.6). Verbatim extraction from CLAUDE.md L410-497 (88 lines pre-trim) covering all engineering gotchas. CLAUDE.md Gotchas section stub cross-refs this canonical source + provides category quick-index. New gotchas ADD here (not back to CLAUDE.md). |
| `superpowers-systematic-debugging` (upstream-imported skill — 4-phase root-cause-first debugging discipline) | `.claude/skills/superpowers-systematic-debugging/SKILL.md` <- imported 2026-05-15 verbatim from `obra/superpowers` v5.1.0 (MIT; https://github.com/obra/superpowers/blob/main/skills/systematic-debugging/SKILL.md) per `_research/superpowers-framework-2026-05-15.md` Option B partial-adoption recommendation. Iron Law: NO FIXES WITHOUT ROOT CAUSE INVESTIGATION FIRST. 4 phases: root-cause investigation → pattern analysis → hypothesis/testing → implementation. Closes gap in project's `.claude/skills/` — no existing `udm-*` skill addresses structured debugging methodology. Renamed YAML `name:` field per project namespace convention; body verbatim per B-280 discipline. |
| `superpowers-verification-before-completion` (upstream-imported skill — pre-completion fresh-evidence-required gate) | `.claude/skills/superpowers-verification-before-completion/SKILL.md` <- imported 2026-05-15 verbatim from `obra/superpowers` v5.1.0 (MIT). Iron Law: NO COMPLETION CLAIMS WITHOUT FRESH VERIFICATION EVIDENCE. Pairs with project's existing `udm-gap-check` (this skill is PRE-completion; gap-check is POST-completion). Direct relevance: would have prevented Pitfall #9.k stale-narrative-quotation pattern from commit `521b68c` (parent agent claimed pytest 2320/62/0 without running pytest; caught only via post-hoc user audit). Renamed YAML `name:` field per project namespace convention. |
| `udm-post-edit-verification` (mandatory post-edit cascade skill) | `.claude/skills/udm-post-edit-verification/SKILL.md` <- introduced 2026-05-15 per user-direction "Turn this into a mandatory event" + CLAUDE.md hard rule 14. 3-step cascade (TEST + GAP ANALYSIS + REVIEW) MANDATORY after any substantive edit (code / markdown / SKILL.md / D-N / RB-N / SP / EventType / config). Anti-triggers: typo (<5 lines + no semantic change), BACKLOG strikethrough-only flip, whitespace, POLISH_QUEUE cosmetic. Commit-message must include explicit verification cascade section OR anti-trigger claim. Closes discipline-debt accumulation pattern (4-event evidence: commit 521b68c stale-narrative-quotation + D.3 + cleanup + D.4 cumulative pragmatic exemptions); supersedes B-286 (Pitfall #9.o formalization candidate) via forward prevention. |
| `superpowers-tdd` (upstream-imported skill — RED-GREEN-REFACTOR test-driven-development) | `.claude/skills/superpowers-tdd/SKILL.md` <- imported 2026-05-15 verbatim from `obra/superpowers` v5.1.0 (MIT). Iron Law: NO PRODUCTION CODE WITHOUT A FAILING TEST FIRST. Lower priority than other 2 Superpowers imports per research recommendation; complements project's `udm-test-author` parallel-agent pattern (which produces tests alongside implementation; superpowers-tdd enforces RED-first if used). Renamed YAML `name:` field per project namespace convention. |
...
| **main** | `tools/archive_chat_session.py` | B-569 closure entry point — SESSION_RESUME/active/<chat>.md archive lifecycle automation; replaces operator-manual `mv` workflow per `SESSION_RESUME/README.md` lifecycle (2026-05-19) |
| **cli_main** | `tools/archive_chat_session.py` | argv parser + dispatcher; required `--chat <name>`; optional `--closure-reason` / `--abandoned <reason>` / `--apply`; per D74 + D75 dry-run-default + D76 audit row (2026-05-19) |
| **archive_chat** | `tools/archive_chat_session.py` | Core archive function; verifies `active/<chat>.md` exists (FileNotFoundError otherwise); computes target via `_compute_archive_filename`; appends `_build_closure_metadata` block (archived-at + final-commit via `_current_head_hash` + chat-name + status + reason); writes to `_archive/`; updates router via `_update_router_active_table` (defensive); deletes source. Returns `(source_path, archive_path, was_applied)` tuple (2026-05-19) |
| **_compute_archive_filename** | `tools/archive_chat_session.py` | Builds archive filename per `SESSION_RESUME/README.md` lifecycle spec: `<YYYY-MM-DD>-<chat>.md` (clean) OR `<YYYY-MM-DD>-<chat>-ABANDONED-<reason-normalized>.md` (abandoned). Reason normalization: lowercase + spaces/underscores → hyphens + non-alphanumeric stripped (2026-05-19) |
| **_update_router_active_table** | `tools/archive_chat_session.py` | Regex-based row-removal helper for root `SESSION_RESUME.md` active-chats table. Pattern matches `| **<chat>** | scope | path |` rows. Defensive: returns True if row not found OR file missing (silent no-op for pre-B-562 Phase 2 repos). Returns False only on OSError (2026-05-19) |
| **EVENT_TYPE** (`= "CLI_ARCHIVE_CHAT_SESSION"`) | `tools/archive_chat_session.py` | Canonical D76 PipelineEventLog.EventType value for archive audit rows; **28th CLI_* family member** per CLAUDE.md L211 registry (B-569 closure 2026-05-19) |
| **EXIT_SUCCESS / EXIT_WARNING / EXIT_OPERATIONAL / EXIT_FATAL** (`= 0 / 1 / 2 / 3`) | `tools/archive_chat_session.py` | D74 canonical exit-code contract; SUCCESS = archive complete OR dry-run preview; FATAL = `active/<chat>.md` missing (FileNotFoundError); OPERATIONAL = filesystem write error (2026-05-19) |
| **main** | `tools/claim_next_bn.py` | B-562 Component A entry point — atomic B-N integer claim CLI; eliminates manual-discipline-dependent BACKLOG re-read after empirical B-N collision 2026-05-19 (this chat `665f14d` vs parallel chat `9b1d7fb` → renumbered at `7a810b9`); wraps `cli_main()`; D74 exit-code contract (2026-05-19) |
| **cli_main** | `tools/claim_next_bn.py` | argv parser + dispatcher; report-only when no `--scope`; `--scope ... --severity ... --wsjf ... --apply` writes placeholder BACKLOG.md entry; per D74 + D75 dry-run-default + D76 audit row (2026-05-19) |
| **find_highest_bn** | `tools/claim_next_bn.py` | Parses BACKLOG.md via `_BN_ROW_RE` matching both standard `- **B-N**` and strikethrough-wrapped `- ~~**B-N**~~` formats per B-490 canonical variants; returns max integer or 0 defensive-fallback on missing/empty file (2026-05-19) |
| **next_available_bn** | `tools/claim_next_bn.py` | Returns `find_highest_bn() + 1` — the next available B-N slot for atomic claim (2026-05-19) |
| **open_placeholder_entry** | `tools/claim_next_bn.py` | Writes `- **B-N** (🟡 Open; <severity>; WSJF <wsjf>): **[PLACEHOLDER — replace with full scope]** <scope>` to BACKLOG.md; raises ValueError on invalid severity; honors `dry_run=True` default per D75 (2026-05-19) |
...
| **CHECKS** | `tools/pre_commit_checks.py` | Registry list of 13 check functions per B-565 closure 2026-05-19 (was 12 pre-B-565 / 11 pre-B-558-C / 10 pre-B-558-A / 9 pre-B-495 / 8 pre-B-481 / 4 at original B-308 closure). Composition: (1) check_query_blindspots / (2) check_pytest_changed_python_files / (3) check_lint_security_types_changed_python_files / (4) check_markdown_cross_refs / (5) check_cli_compliance_d74_d75_d76 / (6) check_gap_accountability / (7) check_planning_provenance / (8) check_cli_registry_sync / (9) check_wc_line_count_claims / (10) check_file_path_existence / (11) check_snapshot_claims / (12) check_snapshot_pytest_claims / (13) check_session_resume_active_refresh. Orchestrator `run_all_checks(staged)` iterates registry + returns `list[CheckResult]`. New checks append (preserves enumeration; existing test pins on `CHECKS[-1] is check_session_resume_active_refresh` per B-565 Tier 0 assertion 1). |
| **check_session_resume_active_refresh** | `tools/pre_commit_checks.py` | Phase 1 quality-check per B-565 closure 2026-05-19 (13th CHECKS registry entry). Pitfall #9.m recursive-self-violation forward-prevention per HANDOFF §8 + 2-event empirical anchor 2026-05-19 (cross-cohort reviewer `ae0e5ea9c1b3851c0` instance N at `c8bb55b..372e982`; gap-check reviewer `a7f466490e1f64dc5` instance N+1 at `739eab1`). Mechanical-enforcement closure of the producer-discipline-only B-558 Step 3 mandate (refresh `SESSION_RESUME/active/<chat-name>.md` after every substantive commit). Logic: if BACKLOG.md in staged files AND staged diff contains ≥1 `+...B-NNN...⚫ CLOSED` line (via `_BACKLOG_CLOSURE_FLIP_RE`) AND no `SESSION_RESUME/active/*.md` file in staged-files list → WARN with B-N enumeration + recommended remediation. Edge case: multi-commit cohorts may defer refresh to trailing commit (WARN expected in that scope). WARN severity per FP-policy precedent. 7 Tier 0 assertions at `tests/tier0/test_pre_commit_checks_b565_active_refresh.py`. |
| **check_snapshot_pytest_claims** | `tools/pre_commit_checks.py` | Phase 1 quality-check per B-558 Phase 2.1 Component C closure 2026-05-19 (12th CHECKS registry entry). Snapshot-pytest-scope-ambiguity forward-prevention per `docs/migration/UDM_SESSION_COMPACTOR_PHASE_2_1_PLAN_2026-05-19.md` §3.3 (Option B chosen per gate-2 reviewer `abbbbd0ae702860da` G3-1). Analog of B-449 commit-msg pytest-count-disambiguation check, applied to snapshot scope. Reuses canonical `_PYTEST_COUNT_RE` + `_SCOPE_INDICATORS` imported from `tools/check_commit_msg.py`. Scans staged `docs/migration/_session_snapshots/*.md` files for pytest count claims (3-5 digits + pass/passed keyword) and verifies each has a co-located (±2 lines) scope indicator. Layer 2 suppression via `is_empirical_anchor_context()` for historical-citation pytest counts. WARN severity (per FP-policy precedent of B-481 + B-495 + B-558 Component A); findings cap 10. 4 Tier 0 assertions at `tests/tier0/test_pre_commit_checks_b558_pytest_claims.py`. |
| **check_snapshot_claims** | `tools/pre_commit_checks.py` | Phase 1 quality-check per B-558 Phase 2.1 Component A closure 2026-05-19 (11th CHECKS registry entry). Snapshot-frontmatter-hallucination forward-prevention per `docs/migration/UDM_SESSION_COMPACTOR_PHASE_2_1_PLAN_2026-05-19.md` §3.1. Scans staged `docs/migration/_session_snapshots/*.md` files for YAML frontmatter `commit_hash:` claims + verifies each cited hash exists in `git log --format=%H` output via `_git_log_contains_hash()` subprocess helper. WARN severity (per FP-policy precedent of B-481 + B-495). v1 scope = commit_hash only; pytest-claim verification = B-558 Phase 2.1 Component C deferred; B-N closure-count cross-check = future enhancement. 8 Tier 0 assertions at `tests/tier0/test_pre_commit_checks_b558_snapshot_claims.py`. |
| **_SNAPSHOT_COMMIT_HASH_RE / _SNAPSHOT_DIR_PREFIX / _git_log_contains_hash** | `tools/pre_commit_checks.py` | Regex constant + path prefix + subprocess helper per B-558 Phase 2.1 Component A — power `check_snapshot_claims`. `_SNAPSHOT_COMMIT_HASH_RE` matches `^commit_hash:\s*([a-f0-9]{7,40})\s*$` (MULTILINE; case-insensitive). `_SNAPSHOT_DIR_PREFIX = "docs/migration/_session_snapshots/"`. `_git_log_contains_hash(candidate)` runs `git log --format=%H` in REPO_ROOT with 10s timeout + iterates output for prefix-match against candidate; returns False defensively on subprocess failure. |
| **check_file_path_existence** | `tools/pre_commit_checks.py` | Phase 1 quality-check per B-495 closure 2026-05-18 (10th CHECKS registry entry). LLM file-path-confabulation forward-prevention per udm-researcher artifact `_research/llm-handoffs-traceability-hallucination-2026-05-18.md` Recommendation 3 + Finding 3.1 (code hallucination systematic review; arXiv 2511.00776; 60-paper meta-analysis 2025 identifies file-path confabulation as LEAST-MITIGATED sub-type). Regex `_BACKTICK_PATH_RE` matches backtick-wrapped path candidates with known repo-directory prefix (`tools/` / `.claude/` / `tests/` / `docs/` / etc.) + known file extension (`.py` / `.md` / `.sql` / `.yml` / etc.) OR trailing `/` (directory); rejects wildcards (`*` `?`) + template placeholders (`<>` `{}`) per FP-policy. `_is_credible_path_candidate(token)` post-filter rejects non-path slash-containing tokens (e.g., `polars/series`). Verifies cited path exists via `(REPO_ROOT / path).exists()`. WARN severity (NOT BLOCK; per FP-policy: D-N proposals may legitimately cite future-planned paths). Silent skip for empty staged-markdown set. |
| **_BACKTICK_PATH_RE / _PATH_PREFIX_WHITELIST / _PATH_EXTENSION_WHITELIST / _is_credible_path_candidate** | `tools/pre_commit_checks.py` | Regex constants + helpers per B-495 closure 2026-05-18 — power `check_file_path_existence`. `_BACKTICK_PATH_RE` matches `` `<path>` `` with no wildcards/placeholders + ≥1 slash. `_PATH_PREFIX_WHITELIST` tuple (~19 entries) covers known repo directories (extensible). `_PATH_EXTENSION_WHITELIST` tuple (11 file extensions). `_is_credible_path_candidate(token)` returns True iff token starts with whitelisted prefix AND (ends with whitelisted extension OR ends with `/`). |
| **EVENT_TYPE** (`= "CLI_PRE_COMMIT_CHECKS"`) | `tools/pre_commit_checks.py` | 18th CLI_* family member per CLAUDE.md L207 (2026-05-16) |
| **check_wc_line_count_claims** | `tools/pre_commit_checks.py` | Phase 1 quality-check per B-481 closure 2026-05-18 (9th CHECKS registry entry). Pitfall #9.h forward-prevention — regex-matches `` `<filename>` (... N lines per actual `wc -l` ...) `` in staged markdown; resolves filename via canonical map `_WC_CANONICAL_FILE_PATHS`; verifies cited count against actual `wc -l`. WARN on mismatch (not BLOCK; LOW WSJF + Mechanism C-1 stale-narrative-class contract). Silent skip for unresolvable filenames per defensive design. Composes with existing markdown cross-ref check pattern. Closes empirical 2026-05-18 drift class (CLAUDE.md L98 cited 127/117 stale post-multiple-refactors; actual 68/41). |
| **_WC_LINE_COUNT_CLAIM_RE / _WC_CANONICAL_FILE_PATHS / _resolve_wc_target_path** | `tools/pre_commit_checks.py` | Regex constants + helper per B-481 closure 2026-05-18 — power `check_wc_line_count_claims`. `_WC_LINE_COUNT_CLAIM_RE` matches `` `<filename>` ([^(]*\(...N lines per actual `wc -l` ...) `` (case-insensitive). `_WC_CANONICAL_FILE_PATHS` tuple maps bare filename tokens to repo-relative paths (currently 2 entries: `pre-commit` → `.githooks/pre-commit`; `commit-msg` → `.githooks/commit-msg`). `_resolve_wc_target_path(filename)` returns resolved path or None for unresolvable tokens. Add new tuple entries as more wc -l claim patterns emerge. |
...
| **CommitMsgCheck** | `tools/check_commit_msg.py` | Abstract base class for commit-msg orchestrator checks per B-459 closure 2026-05-18 + extended via B-466/467/468 completion cohort + B-470/471/472 abstraction-polish cohort 2026-05-18. Subclass contract: declare class attributes `name: str` (unique identifier; used as audit-row JSON findings dict key) + `severity: Literal["WARN","BLOCK"]` + `requires_backlog_diff: bool` + `requires_classification: bool` (per B-472 — declarative replacement for brittle isinstance dispatch); implement `scan(commit_msg, ctx: OrchestrationContext) -> CheckResult` (per B-467 closure; was `scan(commit_msg, staged_diffs)` pre-B-467). `__init_subclass__` hook validates ALL required class attributes at class-definition time (per B-466 — fail-fast hasattr check) AND validates severity VALUE in {"WARN","BLOCK"} canonical literals (per B-471 — closes typo-class failure mode like `severity="BLCK"` that pre-B-471 silently degraded to WARN at orchestrator BLOCK-comparison time). Orchestrator contract: iterate `CHECKS` registry + pass commit_msg + OrchestrationContext (carrying staged_diffs + classification, batched once per main() entry) to each subclass instance; collect findings under `check.name` key in audit-row findings dict; only `severity=BLOCK` contributes to final exit code. Subclass MAY override `render_findings_to_stderr(findings)` (per B-468 — eliminates 4-way copy-paste); base default emits `[<severity>] <name>: <finding>` format. Replaces 4-way copy-paste of `_strip_code_blocks` + WARN-only + try/except + audit-row-extension patterns previously duplicated across exemption-phrase + cascade-evidence + B-449 + B-451 checks. |
| **CheckResult** | `tools/check_commit_msg.py` | Frozen dataclass per B-459 closure 2026-05-18 — per-check `scan()` result with `passed: bool` + `findings: list[str]` (empty when `passed=True`). **MODULE-DISAMBIGUATED PER B-465 (2026-05-18)**: `tools/check_commit_msg.py::CheckResult` (2-field shape) is DISTINCT from `tools/pre_commit_checks.py::CheckResult` (4-field shape per row above — `name + passed + severity + diagnostic`). Both retained for back-compat; cross-module imports MUST use `from tools.check_commit_msg import CheckResult` vs `from tools.pre_commit_checks import CheckResult` to disambiguate. Future refactor hazard if collisions multiply — B-N candidate may rename one to e.g. `CommitMsgCheckResult` if pipeline-lead authorizes breaking change. |
| **ExemptionPhraseCheck** | `tools/check_commit_msg.py` | CommitMsgCheck subclass per B-459 closure 2026-05-18; severity=BLOCK; requires_backlog_diff=False. Scans commit-msg for 12-phrase exemption-trigger list per `udm-exemption-verifier` SKILL.md L29-46 → BLOCKS with reviewer-spawn instruction. Pre-B-459 was top-level `check_exemption_phrases()` function; preserved as thin delegation wrapper for back-compat. |
| **CascadeEvidenceCheck** | `tools/check_commit_msg.py` | CommitMsgCheck subclass per B-459 closure 2026-05-18; severity=BLOCK; requires_backlog_diff=False. Scans commit-msg for hard rule 14 tri-section structure (`## TEST` + `## GAP ANALYSIS` + `## REVIEW`); validates SUBSTRATE_EDIT cascade discipline via `cascade_classifier.has_cascade_evidence(classification=)` compositional pass-through; BLOCKS on missing sections + invalid SKIPPED labels per B-321. |
| **PytestCountDisambiguationCheck** | `tools/check_commit_msg.py` | CommitMsgCheck subclass per B-449 + B-459 closures 2026-05-18; severity=WARN; requires_backlog_diff=False. Scans TEST section for pytest counts cited without co-located scope-indicator (45-substring scope-indicator tuple incl. `tier0+tier1` / `full-suite` / `baseline preserved` / `python -m pytest` / `tests/tier0`); code-block-quoted output excluded via `_strip_code_blocks`; ±3-line context window for disambiguation; WARN-only (does NOT contribute to BLOCK exit code). Pre-B-459 was top-level `check_pytest_count_disambiguation()` function; preserved as thin delegation wrapper. Empirical anchor commit `e76078c`. |
| **UnresolvedForwardPreventionCandidatesCheck** | `tools/check_commit_msg.py` | CommitMsgCheck subclass per B-451 + B-459 closures 2026-05-18; severity=WARN; requires_backlog_diff=True. Scans commit-msg GAP ANALYSIS + REVIEW + body for orphan-candidate trigger phrases via 8-pattern tuple (`deferred (B-N candidate for X)` / `tracked as B-N TBD` / `forward-prevention B-N candidate` / etc.); verifies disposition via either (a) BACKLOG.md staged diff openings matched via `^\+\s*(?:[-*]\s*)?\*\*B-(\d+)\*\*\s*\([^)]*?Open` regex OR (b) explicit dismissal phrasing via 13-phrase dismissal tuple; blockquote + code-fence content excluded to avoid false-positives. WARN-only. Pre-B-459 was top-level `check_unresolved_forward_prevention_candidates()` function; preserved as thin delegation wrapper. Empirical anchor commit `e76078c` GAP "B-409 + B-414 deferred candidate" without BACKLOG opening. |
| **_is_empirical_anchor_context** | `tools/check_commit_msg.py` | Helper per B-488 closure 2026-05-18 — parallel to existing `_is_inside_blockquote()`. Returns True when `lines[idx]` is within 5-line lookback window of any `_EMPIRICAL_ANCHOR_MARKERS` phrase. Suppresses self-reference meta-pattern false-positives across ClosureAnnotationConsistencyCheck + NarrativePytestClaimVerificationCheck (each check's `scan()` calls this after `_is_inside_blockquote()` and skips matches in anchor context). Empirical 3-event evidence base: B-458 self-firing at 133b212 (B-480 candidate) + B-464 self-firing at c6ba969 (B-487 candidate) + B-470 latent vulnerability. Signature: `(lines: list[str], idx: int, lookback: int = 5) -> bool`. |
| **_EMPIRICAL_ANCHOR_MARKERS** | `tools/check_commit_msg.py` | Tuple of 18 canonical anchor-marker phrases per B-488 closure 2026-05-18. Phrases derived from empirical false-positive events at commits 133b212 + c6ba969. Includes both case variants (e.g., "META-IRONY" + "meta-irony" + "empirical anchor commit" + "Quote-cite from reviewer" + "Mechanism A step 5" + "per Cohort" + "verbatim quote" + "Reviewer cited"). Case-sensitive match (case variants enumerated explicitly to avoid regex complexity). Future markers may be appended at next empirical-event evidence. |
| **NarrativePytestClaimVerificationCheck** | `tools/check_commit_msg.py` | CommitMsgCheck subclass per B-464 closure 2026-05-18; severity=WARN; requires_backlog_diff=False; requires_classification=False. 7th orchestrator check. Heuristic narrative pytest-claim verification — catches anomalously high skip-counts indicating copy-paste-stale narrative or arithmetic error. 1st-event empirical anchor: commit `1f74b72` META-IRONY pattern (`2664 pass / 62 skip` cited in 4 locations; actual cascade scope returned `2664 pass / 10 skip` — 6.2x drift; root cause copy-paste-stale narrative). Detection: `_PYTEST_FULL_TRIPLET_RE` matches `N pass / M skip [/ K fail]` patterns (case-insensitive; `/` or `,` separator); WARN when `M > _PYTEST_SKIP_ANOMALY_THRESHOLD` (=20; 2x project baseline ~10). Code-block + blockquote suppression via existing helpers. Findings emitted in line-number order (natural iteration top-to-bottom; not explicitly sorted); capped at 10. Conservative WARN heuristic threshold permits legitimate skip-count growth without false positives. Orthogonal-failure-mode complement to B-449 PytestCountDisambiguationCheck (B-449 catches missing-scope; B-464 catches anomalous-value). Subprocess `pytest --collect-only` approach considered + rejected for ~5-10s overhead exceeding reasonable hook latency; future B-N may add dynamic-baseline verification if heuristic threshold proves insufficient. |
| **_PYTEST_FULL_TRIPLET_RE / _PYTEST_SKIP_ANOMALY_THRESHOLD** | `tools/check_commit_msg.py` | Regex + threshold constants per B-464 closure 2026-05-18 — power the NarrativePytestClaimVerificationCheck. `_PYTEST_FULL_TRIPLET_RE` matches `\b(?P<pass>\d{2,5})\s*pass[/,]\s*(?P<skip>\d{1,4})\s*skip(?:\s*[/,]\s*(?P<fail>\d{1,3})\s*fail)?` case-insensitive with named capture groups. `_PYTEST_SKIP_ANOMALY_THRESHOLD = 20` calibrated at 2x project baseline (~10 skipped on tier0+tier1+unit+property+regression scope). |
| **ClosureAnnotationConsistencyCheck** | `tools/check_commit_msg.py` | CommitMsgCheck subclass per B-458 closure 2026-05-18; severity=WARN; requires_backlog_diff=True; requires_classification=False. 6th orchestrator check. Detects retrospective B-N CLOSED claims in commit-msg without corresponding `**B-NNN** (⚫ CLOSED` annotation in staged BACKLOG.md diff. Empirical anchor: commit `20fe33a` claimed `**B-409 CLOSED**` + `**B-414 CLOSED**` in subject + body but BACKLOG diff applied closure annotation ONLY to B-408 — required Agent 58 gap-check + commit `e76078c` remediation cycle to backfill. Detection logic: `_CLOSURE_CLAIM_RE` matches `**B-NNN CLOSED**` (bold form) OR bare `B-NNN CLOSED` / `B-NNN ⚫ CLOSED` (case-insensitive); `_BACKLOG_CLOSURE_ANNOTATION_RE` matches `^\+.*\*\*B-NNN\*\* (⚫ CLOSED` per-line in BACKLOG.md staged-diff (MULTILINE mode). Code-block suppression via `_strip_code_blocks`; blockquote suppression via `_is_inside_blockquote`. Findings sorted numerically by B-N; capped at 10. Conservative WARN heuristic: silently passes when no BACKLOG.md staged (cannot verify). Orthogonal-failure-mode complement to B-451 `UnresolvedForwardPreventionCandidatesCheck` — B-451 catches forward-looking orphan-candidate phrasings; B-458 catches retrospective closure claims; both share `_collect_staged_diffs` BACKLOG batching via `requires_backlog_diff=True`. |
| **_CLOSURE_CLAIM_RE / _BACKLOG_CLOSURE_ANNOTATION_RE** | `tools/check_commit_msg.py` | Regex constants per B-458 closure 2026-05-18 — power the ClosureAnnotationConsistencyCheck parser. `_CLOSURE_CLAIM_RE` matches commit-msg B-N CLOSED claims via alternation: bold `\*\*B-(\d+)\s+CLOSED\*\*` (group 1) OR bare `\bB-(\d+)\s+(?:⚫\s*)?CLOSED\b` (group 2; allows optional `⚫` prefix). Case-insensitive on CLOSED literal. `_BACKLOG_CLOSURE_ANNOTATION_RE` matches per-line annotations in BACKLOG.md staged-diff via `^\+.*\*\*B-(\d+)\*\*\s*\(\s*⚫\s*CLOSED` (MULTILINE mode); `^\+` anchors to git-diff addition lines. |
| **InlineFixClaimVerificationCheck** | `tools/check_commit_msg.py` | CommitMsgCheck subclass per B-470 closure 2026-05-18; severity=WARN; requires_backlog_diff=False; requires_classification=False. Forward-prevention for "claim-vs-reality drift" — PRE-COMMIT reviewer inline-fix claims that DO NOT actually land in the staged file state. 2-event empirical evidence base 2026-05-18: commit `2a33efa` Agent 70 B-459 leading-badge claim (caught by Agent 71 at `7eef2ef`) + commit `20d998f` Agent 74 B-465 + GLOSSARY signature claims (caught by Agent 75 at `9775340`). Root cause: parent Edit operations silently overwritten by subsequent re-Read+re-Edit cycles when files modified between calls. Detection logic: extracts reviewer-block via `_REVIEWER_BLOCK_HEADER_RE` regex (`Independent.*reviewer Agent N (...)`); enumerates numbered-fix items via `_NUMBERED_FIX_RE`; classifies by Pitfall #9.X letter (`_PITFALL_MARKER_RE`) into kind in {`badge_flip` (Pitfall #9.j), `transition` (Pitfall #9.k or #9.l), `missing_entries` (Pitfall #9.n), `unknown`}; resolves target file via `_resolve_target_path` against `_CANONICAL_FILE_PATHS` tuple; fetches staged content via `_fetch_staged_content` (`git show :<path>`). Pitfall #9.j badge_flip verification: checks `**B-NNN** (🟡 Open` is NOT present in staged content for cited B-N. Pitfall #9.k/#9.l transition verification: checks "<after>" substring IS present in staged target file. Pitfall #9.n missing_entries verification: deferred (parser support only). WARN-only per WSJF MEDIUM (matches B-449 + B-451 contract). Closes 2-event recurrence pattern + Pitfall #9.q candidate; escalation to BLOCK reserved for pipeline-lead post-baseline period. |
| **_extract_reviewer_block** | `tools/check_commit_msg.py` | Helper per B-470 closure 2026-05-18 — returns substring of commit-msg starting at first match of `_REVIEWER_BLOCK_HEADER_RE`. Empty string when no reviewer-block header present. Signature: `(commit_msg: str) -> str`. |
| **_parse_inline_fix_claims** | `tools/check_commit_msg.py` | Helper per B-470 closure 2026-05-18 — parses numbered fix items from reviewer-block text. Returns `list[dict]` each with keys `fix_num` (1-based int) + `raw_body` (concatenated body lines) + `kind` (`"badge_flip"` / `"transition"` / `"missing_entries"` / `"unknown"`) + `before` + `after` + `bn` (B-N anchor) + `target_path` (canonical repo path). Tolerates prose between closing-quote and arrow (canonical form per forensic analysis of commits `2a33efa` + `20d998f`). Signature: `(reviewer_block: str) -> list[dict]`. |
| **_resolve_target_path** | `tools/check_commit_msg.py` | Helper per B-470 closure 2026-05-18 — maps filename tokens in fix-body (BACKLOG / GLOSSARY / CLAUDE / CURRENT_STATE / HANDOFF / SESSION_RESUME with-or-without ".md" suffix) to canonical repo paths via `_CANONICAL_FILE_PATHS` tuple. Longer ".md" forms preferred (substring-match wins). Returns None when no canonical filename token found. Signature: `(fix_body: str) -> str \| None`. |
| **_fetch_staged_content** | `tools/check_commit_msg.py` | Helper per B-470 closure 2026-05-18 — fetches staged-content of a file via `git show :<path>` subprocess (reads from git index, NOT working-tree). Empty string on failure (file not in index OR git unavailable). Caller MUST cache per path; each call spawns subprocess. Signature: `(path: str) -> str`. Monkeypatched in Tier 0 tests to simulate staged-content variations without git-state setup. |
| **_MISSING_ENTRIES_RE / _IDENT_SEPARATOR_RE** | `tools/check_commit_msg.py` | Regex constants per B-477 closure 2026-05-18 — power the `InlineFixClaimVerificationCheck.scan()` Pitfall #9.n missing_entries kind verification branch. `_MISSING_ENTRIES_RE` matches `missing N NEW <scope> surfaces (<ident-list>)` format (canonical per commit 20d998f Fix #3); captures parenthesized ident-list. `_IDENT_SEPARATOR_RE` splits ident-list on `+` / `,` / `;` / `and` separators. Identifiers shorter than 3 characters skipped (false-positive suppression for short tokens). |
| **_REVIEWER_BLOCK_HEADER_RE / _NUMBERED_FIX_RE / _PITFALL_MARKER_RE / _BN_REF_RE / _TRANSITION_RE / _CANONICAL_FILE_PATHS** | `tools/check_commit_msg.py` | Regex constants + canonical filename tuple per B-470 closure 2026-05-18 — power the InlineFixClaimVerificationCheck parser. `_REVIEWER_BLOCK_HEADER_RE` matches `Independent.*reviewer Agent N (...)` (tolerant variants). `_NUMBERED_FIX_RE` matches `^\d+\.\s+...` numbered items. `_PITFALL_MARKER_RE` extracts Pitfall #9.X letter. `_BN_REF_RE` extracts B-N identifier. `_TRANSITION_RE` matches `"old" ... → ... "new"` allowing prose between (lazy non-greedy match). `_CANONICAL_FILE_PATHS` tuple maps filename tokens to repo paths. |
| **CHECKS** | `tools/check_commit_msg.py` | Registry list of 7 CommitMsgCheck subclass instances per B-459 + B-470 + B-458 + B-464 closures 2026-05-18 — `ExemptionPhraseCheck()` + `CascadeEvidenceCheck()` + `PytestCountDisambiguationCheck()` + `UnresolvedForwardPreventionCandidatesCheck()` + `InlineFixClaimVerificationCheck()` + `ClosureAnnotationConsistencyCheck()` + `NarrativePytestClaimVerificationCheck()`. Orchestrator `main()` iterates registry; future check additions (e.g. B-464 narrative pytest-claim verification) land as single `CommitMsgCheck` subclass + `CHECKS` registry append without orchestrator changes. **NAME COLLISION NOTE PER B-465 (2026-05-18)**: `tools/check_commit_msg.py::CHECKS` (list[CommitMsgCheck]) is DISTINCT from `tools/pre_commit_checks.py::CHECKS` (tuple of 8 check functions per L765 row); cross-module imports MUST disambiguate via module-prefixed path. |
| **_collect_staged_diffs** | `tools/check_commit_msg.py` | Helper per B-459 closure 2026-05-18 — batches `git diff --cached` fetch for files required by enabled checks. Used by orchestrator before iterating `CHECKS`; avoids redundant subprocess calls when multiple checks need the same diff. Currently the only diff-needing file is `docs/migration/BACKLOG.md` (used by `UnresolvedForwardPreventionCandidatesCheck` + future `ClosureAnnotationConsistencyCheck` per B-458). Signature: `(checks: list[CommitMsgCheck]) -> dict[str, str]`. |
| **EVENT_TYPE** (`= "CLI_CHECK_COMMIT_MSG"`) | `tools/check_commit_msg.py` | 24th CLI_* family member per CLAUDE.md L207 (registered 2026-05-17 via Option A mechanical enforcement sweep when authoring `check_cli_registry_sync`); canonical D76 `PipelineEventLog.EventType` value for commit-msg-check audit rows. |
| **EXIT_SUCCESS / EXIT_BLOCKED** (`= 0 / 1`) | `tools/check_commit_msg.py` | D74 exit-code contract — SUCCESS = all BLOCK-severity checks pass; BLOCKED = any BLOCK-severity finding (WARN-severity findings emit stderr + audit-row but do NOT contribute to exit code per WSJF MEDIUM contract; pipeline-lead escalation reserved). |
| **REPO_ROOT** | `tools/check_commit_msg.py` | Path constant — resolved repo root via `Path(__file__).resolve().parent.parent` (tools/check_commit_msg.py → tools/ → repo). Used by `_collect_staged_diffs` for `git diff --cached` subprocess cwd anchoring. |
| **OrchestrationContext** | `tools/check_commit_msg.py` | Frozen dataclass per B-467 closure 2026-05-18 — batches `staged_diffs: dict[str, str]` + `classification: CommitClassification \| None = None` ONCE at orchestrator entry; passed to every CommitMsgCheck.scan() invocation eliminating per-check redundant external-state collection (verified `classify_commit()` subprocess invocations reduced 2→1 per main() run via test_main_calls_classify_commit_only_once Tier 0 assertion). |
| **_build_orchestration_context** | `tools/check_commit_msg.py` | Helper per B-467 closure 2026-05-18 + extended via B-472 closure 2026-05-18 — builds canonical OrchestrationContext once per main() invocation. Signature: `(checks: list[CommitMsgCheck]) -> OrchestrationContext`. Calls `classify_commit()` when ANY check declares `requires_classification = True` (per B-472 — declarative dispatch via `any(getattr(c, "requires_classification", False) for c in checks)` REPLACES brittle `isinstance(c, CascadeEvidenceCheck)` dispatch). Skip optimization for synthetic/test contexts without cascade-aware checks; graceful exception swallow returns `classification=None`. Replaces pre-B-467 `_collect_staged_diffs(checks)` helper which only batched staged_diffs (B-467 extended to also batch classification). |
...
| **main** | `tools/cascade_classifier.py` | Hard rule 14 cascade classifier entry (B-317 Phase 1B; 2026-05-16); wraps `cli_main()`; D74 exit-code contract |
| **cli_main** | `tools/cascade_classifier.py` | argv parser + dispatcher; `--json` flag for machine-readable; D74 exit codes |
| **classify_commit** | `tools/cascade_classifier.py` | Programmatic entry — classifies staged scope into one of 6 labels (SUBSTRATE_EDIT / TYPO_ONLY / WHITESPACE_ONLY / BADGE_FLIP_ONLY / POLISH_QUEUE_ONLY / SUBSTANTIVE); returns `CommitClassification` dataclass; strict mode: substrate-edits OVERRIDE all anti-trigger detection per CLAUDE.md hard rule 14 substrate-edit clause (2026-05-16) |
| **has_cascade_evidence** | `tools/cascade_classifier.py` | Programmatic entry — verifies commit message has tri-section structure (`## TEST` + `## GAP ANALYSIS` + `## REVIEW`) per B-318 + body-content validation per B-321; optional `classification=` kwarg enables substrate-stricter REVIEW check + SKIPPED label-prefix check (2026-05-16; extended B-321 closure 2026-05-17) |
| **is_substrate_path** | `tools/cascade_classifier.py` | Predicate — true if file_path is in enumerated substrate per `SUBSTRATE_FILES` exact-match OR `SUBSTRATE_DIR_PREFIXES` prefix-match; mechanically detects enforcement-discipline files NEVER anti-trigger (2026-05-16) |
| **CommitClassification** | `tools/cascade_classifier.py` | Dataclass: `classification / rationale / is_anti_trigger / cascade_required / staged_count / total_lines_changed`; `to_dict()` for JSON serialization (2026-05-16) |
| **SUBSTRATE_FILES** | `tools/cascade_classifier.py` | Frozenset of 16 substrate file paths (hook orchestrator + git hooks + CI mirror + discipline canon + tools/cascade_classifier itself); NEVER anti-trigger per Phase 2A clause (2026-05-16) |
| **SUBSTRATE_DIR_PREFIXES** | `tools/cascade_classifier.py` | Tuple of 4 substrate directory prefixes (`.claude/skills/udm-` / `.claude/agents/udm-` / `.claude/hooks/` / `docs/migration/blindspots/`); prefix-match for skill+agent+hook substrate detection (2026-05-16) |
| **CANONICAL_SOURCE_FILES** | `tools/cascade_classifier.py` | Frozenset of canonical D-N/B-N/R-N/RB-N source files; excluded from TYPO_ONLY auto-anti-trigger (small edits to D-N body ARE substantive) (2026-05-16) |
...
### Tier 0 test scaffolding (B-461 closure 2026-05-18)

Shared SKILL.md regression-test scaffolding extracted per B-461 closure to eliminate boilerplate across 4 existing `test_skill_*.py` files (round_closeout / checks_and_balances / edge_case_validator / gap_check) + future SKILL.md Tier 0 pin cohorts. Module is internal test infrastructure; included in GLOSSARY per B-465 closure 2026-05-18 because the 8 public helpers ARE consumed by other test modules (constituting an internal-but-cross-module public surface). Module file's own docstring L40-42 caveat ("no public API surface added to production code") refers to production-code-import surface; cross-test-module imports remain bound by GLOSSARY registration discipline per B-465.

| Identifier | Module | Purpose |
|---|---|---|
| **REPO_ROOT** | `tests/tier0/_skill_test_base.py` | Path constant per B-461 closure 2026-05-18 — resolved repo root via `Path(__file__).resolve().parent.parent.parent` (tests/tier0/_skill_test_base.py → tests/tier0/ → tests/ → repo). Anchor for `get_skill_path()` SKILL.md path composition. |
| **get_skill_path** | `tests/tier0/_skill_test_base.py` | Helper per B-461 closure 2026-05-18 — returns `<REPO_ROOT>/.claude/skills/<skill_name>/SKILL.md` for given skill name. Signature: `(skill_name: str) -> Path`. |
| **make_skill_content_fixture** | `tests/tier0/_skill_test_base.py` | Factory per B-461 closure 2026-05-18 — returns module-scoped pytest fixture that reads SKILL.md content once per test module. Signature: `(skill_name: str) -> Callable`. Caller assigns to module-level name (typically `skill_content`) so pytest discovers it; matches pre-B-461 manual fixture pattern in existing `test_skill_*.py` files. |
| **CanonicalAssertion** | `tests/tier0/_skill_test_base.py` | Frozen dataclass per B-461 closure 2026-05-18 — declarative spec for a SKILL.md content assertion with `name + must_contain + must_match_regex + failure_hint` fields. Forward-extensible per reviewer 🟡 IMPROVE — handles multi-substring / multi-regex cases in a single entry without per-call duplication. |
| **assert_skill_contains_substrings** | `tests/tier0/_skill_test_base.py` | Helper per B-461 closure 2026-05-18 — assertion that SKILL.md content contains ALL named substrings; emits failure_hint on missing match. Signature: `(skill_content: str, substrings: list[str], hint: str = "") -> None`. |
| **assert_skill_matches_regexes** | `tests/tier0/_skill_test_base.py` | Helper per B-461 closure 2026-05-18 — assertion that SKILL.md content matches ALL named regex patterns; emits failure_hint on missing match. Signature: `(skill_content: str, patterns: list[str], hint: str = "") -> None`. |
| **make_baseline_test_skill_exists** | `tests/tier0/_skill_test_base.py` | Factory per B-461 closure 2026-05-18 — produces `test_skill_file_exists` assertion bound to given skill name. Returned callable asserts `get_skill_path(skill_name).is_file()`. Signature: `(skill_name: str) -> Callable`. |
| **make_baseline_test_frontmatter_name** | `tests/tier0/_skill_test_base.py` | Factory per B-461 closure 2026-05-18 — produces `test_frontmatter_name` assertion bound to given skill name. Returned callable asserts SKILL.md frontmatter `name: <skill_name>` matches. Signature: `(skill_name: str) -> Callable`. |
| **REPO_ROOT** | `tests/tier0/_tier0_test_base.py` | Path constant per B-469 closure 2026-05-18 — resolved repo root via `Path(__file__).resolve().parent.parent.parent` (tests/tier0/_tier0_test_base.py → tests/tier0/ → tests/ → repo). Parallel to B-461 `_skill_test_base.py::REPO_ROOT` for CLI tool factory domain. Also ensures `sys.path` includes REPO_ROOT so factory-generated test functions can `importlib.import_module("tools.X")` cleanly. |
| **make_baseline_test_module_imports** | `tests/tier0/_tier0_test_base.py` | Factory per B-469 closure 2026-05-18 — produces `test_module_imports` baseline assertion per D67 Tier 0 import contract. Returned callable invokes `importlib.import_module(module_name)` + raises `AssertionError` with diagnostic message citing D67 if import fails. Catches silent rename/move regression. Signature: `(module_name: str) -> Callable`. Empirical pattern across ~24 CLI tools where each test file repeats the import-assertion boilerplate; this factory generalizes the pattern. |
| **make_baseline_test_event_type_constant** | `tests/tier0/_tier0_test_base.py` | Factory per B-469 closure 2026-05-18 — produces `test_event_type_constant` baseline assertion per D76 audit-row contract. Returned callable asserts `mod.EVENT_TYPE == expected_event_type` (catches silent rename of canonical CLI_* family member). Signature: `(module_name: str, expected_event_type: str) -> Callable`. Composes with `check_cli_registry_sync` pre-commit check which enforces CLAUDE.md L207 CLI_* family registry consistency. |
| **make_baseline_test_exit_codes** | `tests/tier0/_tier0_test_base.py` | Factory per B-469 closure 2026-05-18 — produces `test_exit_codes_canonical` baseline assertion per D74 exit-code contract. Returned callable asserts `mod.EXIT_SUCCESS == expected_exit_success` (default 0) + `mod.EXIT_FATAL == expected_exit_fatal` (default 2). Parameterized to accommodate empirical EXIT_FATAL variance — `tools.query_blindspots` uses `EXIT_FATAL=3` while most other tools use `EXIT_FATAL=2` (D74 baseline). Override via `expected_exit_fatal=3` for query_blindspots-style tools. Signature: `(module_name: str, expected_exit_success: int = 0, expected_exit_fatal: int = 2) -> Callable`. |
...
| **EVENT_TYPE** (`= "CLI_CLAIM_NEXT_BN"`) | `tools/claim_next_bn.py` | Canonical D76 `PipelineEventLog.EventType` value for B-N claim audit rows; **27th CLI_* family member** per CLAUDE.md L210 registry (B-562 Component A 2026-05-19) |
| **EXIT_SUCCESS / EXIT_WARNING / EXIT_OPERATIONAL / EXIT_FATAL** (`= 0 / 1 / 2 / 3`) | `tools/claim_next_bn.py` | D74 canonical exit-code contract; SUCCESS = next slot reported or placeholder written; OPERATIONAL = write attempted but OSError (e.g., read-only filesystem); FATAL = invalid severity OR BACKLOG.md read error (2026-05-19) |
| **VALID_SEVERITIES** (`= ("CRITICAL", "HIGH", "MEDIUM", "LOW")`) | `tools/claim_next_bn.py` | Canonical severity tuple per BACKLOG.md convention; `open_placeholder_entry` raises ValueError if severity argv not in this tuple (2026-05-19) |
| **_BN_ROW_RE** (`= re.compile(r"^- (~~)?\*\*B-(\d+)\*\*", re.MULTILINE)`) | `tools/claim_next_bn.py` | Multi-line regex matching both standard `- **B-N**` AND strikethrough-wrapped `- ~~**B-N**~~` per B-490 canonical format-variants; captures (~~, number) groups; used by `find_highest_bn()` (2026-05-19) |
| **BACKLOG_PATH / SESSION_LOG_DIR** | `tools/claim_next_bn.py` | Path constants — `BACKLOG_PATH` = `<repo>/docs/migration/BACKLOG.md` (single source-of-truth for B-N enumeration); `SESSION_LOG_DIR` = `<repo>/_session_logs/` (D76 audit-row destination) (2026-05-19) |
