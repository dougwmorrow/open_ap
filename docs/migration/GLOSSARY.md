...
| **CHECKS** | `tools/pre_commit_checks.py` | Registry list of 10 check functions per B-495 closure 2026-05-18 (was 9 pre-B-495 / 8 pre-B-481 / 4 at original B-308 closure). Composition: (1) check_query_blindspots / (2) check_pytest_changed_python_files / (3) check_lint_security_types_changed_python_files / (4) check_markdown_cross_refs / (5) check_cli_compliance_d74_d75_d76 / (6) check_gap_accountability / (7) check_planning_provenance / (8) check_cli_registry_sync / (9) check_wc_line_count_claims / (10) check_file_path_existence. Orchestrator `run_all_checks(staged)` iterates registry + returns `list[CheckResult]`. New checks append (preserves enumeration; existing test pins on `CHECKS[-1] is check_file_path_existence` per B-495 Tier 0 assertion 2). |
