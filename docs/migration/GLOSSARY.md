...
| **CHECKS** | `tools/check_commit_msg.py` | Registry list of 5 CommitMsgCheck subclass instances per B-459 + B-470 closures 2026-05-18 — `ExemptionPhraseCheck()` + `CascadeEvidenceCheck()` + `PytestCountDisambiguationCheck()` + `UnresolvedForwardPreventionCandidatesCheck()` + `InlineFixClaimVerificationCheck()`. Orchestrator `main()` iterates registry; future check additions (e.g. B-458 `ClosureAnnotationConsistencyCheck`) land as single `CommitMsgCheck` subclass + `CHECKS` registry append without orchestrator changes. **NAME COLLISION NOTE PER B-465 (2026-05-18)**: `tools/check_commit_msg.py::CHECKS` (list[CommitMsgCheck]) is DISTINCT from `tools/pre_commit_checks.py::CHECKS` (tuple of 8 check functions per L765 row); cross-module imports MUST disambiguate via module-prefixed path. |
