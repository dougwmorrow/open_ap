...
| **CHECKS** | `tools/pre_commit_checks.py` | Registry list of 12 check functions per B-558 Phase 2.1 Component C closure 2026-05-19 (was 11 pre-Component-C / 10 pre-Component-A / 9 pre-B-495 / 8 pre-B-481 / 4 at original B-308 closure). Composition: (1) check_query_blindspots / (2) check_pytest_changed_python_files / (3) check_lint_security_types_changed_python_files / (4) check_markdown_cross_refs / (5) check_cli_compliance_d74_d75_d76 / (6) check_gap_accountability / (7) check_planning_provenance / (8) check_cli_registry_sync / (9) check_wc_line_count_claims / (10) check_file_path_existence / (11) check_snapshot_claims / (12) check_snapshot_pytest_claims. Orchestrator `run_all_checks(staged)` iterates registry + returns `list[CheckResult]`. New checks append (preserves enumeration; existing test pins on `CHECKS[-1] is check_snapshot_pytest_claims` per B-558 Component C Tier 0 assertion 1). |
