...
def _build_safe_select_with_uri(
    ...
    exclude_upper = {c.upper() for c in exclude_columns} if exclude_columns else set()

    if not date_columns and not exclude_upper:
    ...
    try:
        ...
        logger.warning(
            "E-4: Could not get column list for %s.%s — falling back to SELECT *",
            schema, table,
        )
        base = f"SELECT {hint} * FROM {schema}.{table}"
        return f"{base} {where_clause}".strip()

    # Apply exclusions (Oracle returns uppercase column names)
    if exclude_upper:
        before_count = len(all_columns)
        all_columns = [c for c in all_columns if c.upper() not in exclude_upper]
        dropped = before_count - len(all_columns)
        if dropped:
            logger.info(
                "E-XX: Excluding %d column(s) from %s.%s Oracle extraction: %s",
                dropped, schema, table, sorted(exclude_columns),
            )
...
def _supplement_null_partition_rows(
    ...
    try:
        ...
        if len(df_nulls) > 0:
            ...
            # T-2: Use safe_concat instead of diagonal_relaxed
