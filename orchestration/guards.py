...
def check_extraction_guard(
    ...
    *,
    ...
    max_rows_per_day_override: int | None = None,
    context: str = "",
) -> bool:
    ...
    Args:
        ...
        max_rows_per_day_override: Per-table override from
            ``UdmTablesList.MaxRowsPerDay``. When set:

            * The growth-guard limit becomes ``max(growth_threshold *
              baseline, max_rows_per_day_override)`` — Cartesian-join
              detection still blocks anything above the absolute ceiling
              while legitimate growth past the multiplier no longer
              fires (e.g. CARDTXN at ~280k rows/day vs a 2022 baseline
              of ~500 rows).
            * On first run (no baseline), replaces ``first_run_ceiling``
              outright.

            NULL preserves the global default — every existing table
            without this column populated behaves identically to before.
    ...
    if baseline_count is None or baseline_count == 0:
        # No previous run — enforce absolute ceiling. Per-table override
        # takes precedence over the global default.
        ceiling = max_rows_per_day_override if max_rows_per_day_override else first_run_ceiling
        if fresh_count > ceiling:
            logger.error(
                "EXTRACTION GUARD: %s.%s%s first extraction has %d rows "
                "(absolute ceiling=%d). Use --force to override.",
                source_name, table_name, ctx, fresh_count, ceiling,
            )
            ...
            logger.info("O-2_GUARD: %s", json.dumps({
                ...
                "fresh_count": fresh_count, "ceiling": ceiling,
                "ceiling_source": (
                    "max_rows_per_day_override" if max_rows_per_day_override
                    else "first_run_ceiling"
                ),
                "blocked": True,
            }))
            return False
    ...
    # Growth guard — block. Per-table override raises the floor so
    # legitimate growth (e.g. CARDTXN ~500/day in 2022 → ~280k/day in
    # 2024) doesn't fire the multiplier check, while Cartesian-class
    # spikes above the absolute ceiling still block.
    multiplier_limit = int(baseline_count * growth_threshold)
    if max_rows_per_day_override:
        growth_limit = max(multiplier_limit, max_rows_per_day_override)
        limit_source = "max_rows_per_day_override" if growth_limit == max_rows_per_day_override else "multiplier"
    else:
        growth_limit = multiplier_limit
        limit_source = "multiplier"

    if fresh_count > growth_limit:
        logger.error(
            "EXTRACTION GUARD: %s.%s%s row count spiked from %d to %d "
            "(threshold=%d, source=%s). Possible Cartesian join, missing "
            "WHERE clause, or permission change. Skipping to prevent OOM "
            "/ runaway load. Use --force to override.",
            source_name, table_name, ctx,
            baseline_count, fresh_count,
            growth_limit, limit_source,
        )
        logger.info("O-2_GUARD: %s", json.dumps({
            "signal": "extraction_guard", "source": source_name,
            ...
            "growth_limit": growth_limit, "limit_source": limit_source,
            "blocked": True,
        }))
        return False
