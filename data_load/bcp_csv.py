...
def prepare_dataframe_for_bcp(
    ...
    Args:
        ...
        exclude_from_hash: R-10.2 — column names dropped from the change-detection
            hash (e.g. ``['DATELASTMAINT']``). Columns are still extracted and loaded
            to Stage/Bronze normally; they just don't contribute to ``_row_hash``.
            Source: ``TableConfig.exclude_from_hash`` (``UdmTablesList.ExcludeFromHash``).
    ...
    df = add_row_hash(df, source_is_oracle=source_is_oracle, exclude_cols=exclude_from_hash)
