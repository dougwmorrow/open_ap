...
from utils.connections import quote_identifier, get_connection

logger = logging.getLogger(__name__)


# SQL Server type -> Polars dtype mapping (inverse of table_creator._DTYPE_MAP)
_SQL_TO_POLARS: dict[str, pl.DataType] = {
    "BIGINT": pl.Int64,
    "INT": pl.Int32,
    "SMALLINT": pl.Int16,
    "TINYINT": pl.Int8,
    "FLOAT": pl.Float64,
    "REAL": pl.Float32,
    "BIT": pl.Int8,          # BIT -> Int8 (matches C-3b normalize_boolean_to_int8)
    "DATE": pl.Date,
    ...
    "CHAR": pl.Utf8,
    "VARBINARY": pl.Binary,
    "DECIMAL": pl.Float64,   # ConnectorX reads DECIMAL as Float64
    "NUMERIC": pl.Float64,
    "MONEY": pl.Float64,
    "SMALLMONEY": pl.Float64,
}


def sql_type_to_polars(data_type: str) -> pl.DataType:
    """Map a SQL Server base type to a Polars dtype.

    Used by stabilize_extraction_dtypes() to cast extraction DataFrames
    to match the authoritative Stage schema.

    Args:
        data_type: SQL Server base type from INFORMATION_SCHEMA (e.g., 'bigint').

    Returns:
        Polars dtype. Falls back to pl.Utf8 for unknown types.
    """
    return _SQL_TO_POLARS.get(data_type.upper(), pl.Utf8)


def get_stage_polars_schema(full_table_name: str) -> dict[str, pl.DataType]:
    """Build a {column_name: polars_dtype} dict from Stage INFORMATION_SCHEMA.

    Only includes source columns (excludes internal _ prefixed columns).
    Used as the cast target for stabilize_extraction_dtypes().

    Args:
        full_table_name: Fully qualified Stage table name.

    Returns:
        Dict mapping column names to Polars dtypes.
    """
    meta = get_column_metadata(full_table_name)
    return {
        m.column_name: sql_type_to_polars(m.data_type)
        for m in meta
        if not m.column_name.startswith("_")
    }
