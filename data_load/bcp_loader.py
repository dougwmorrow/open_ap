...
class BCPDiagnosticSnapshot:
    ...
    # TODO-2: Session-level cumulative row count (dm_exec_sessions.row_count).
    # Unlike bcp_row_count (request-level, shows last batch = 800), this
    # accumulates across all committed batches during the session.
    bcp_session_rows: int | None = None
    ...
    def to_log_dict(self) -> dict:
        ...
        if self.bcp_session_id is not None:
            ...
            if self.bcp_session_rows is not None:
                d["bcp_cumul_rows"] = self.bcp_session_rows
        else:
...
class _BCPHangMonitor(threading.Thread):
    ...
    def __init__(self, full_table_name: str, poll_interval: float | None = None):
        ...
        # TODO-2: Use session-level cumulative row count instead of
        # dm_exec_requests.row_count which only shows last batch commit.
        self._last_row_count: int | None = None
        self._row_stall_since: float | None = None

        # TODO-4: Track log_used_pct delta as secondary progress indicator.
        # When row count is unreliable (small batch sizes), rising log usage
        # proves BCP is still writing data even if bcp_rows appears frozen.
        self._last_log_used_pct: float | None = None
        self._log_advancing: bool = False
    ...
    def safe_join(self, timeout: float = 5) -> None:
        """Join the monitor thread only if it was actually started.

        Python 3.12 raises RuntimeError if Thread.join() is called on a
        thread that was never started (e.g. when skip_monitor is True).
        This wrapper makes join idempotent and safe in all code paths.
        """
        if self.ident is not None:  # ident is set when .start() is called
    ...
    def _query_bcp_session(self, cur, snap: BCPDiagnosticSnapshot):
        ...
        TODO-2 FIX: Query BOTH dm_exec_requests.row_count (per-batch) and
        dm_exec_sessions.row_count (cumulative). With BCP_BRONZE_BATCH_SIZE=800,
        requests.row_count always shows 800 (last batch). sessions.row_count
        accumulates across all committed batches — the correct signal for
        progress tracking in §5 stall detection.
        """
        cur.execute(
            "SELECT TOP 1 "
            "  s.session_id, r.status, r.wait_type, r.wait_time, "
            "  r.wait_resource, r.blocking_session_id, r.row_count, "
            "  r.total_elapsed_time, s.row_count "
        ...
        if row:
            ...
            # TODO-2: Session-level cumulative row count for §5 stall detection
            snap.bcp_session_rows = row[8]
    ...
    def _evaluate_hang(self, snap: BCPDiagnosticSnapshot, elapsed: float):
        ...
        #
        # TODO-2 FIX: dm_exec_requests.row_count shows the last committed
        # batch count, NOT cumulative rows. With BCP_BRONZE_BATCH_SIZE=800,
        # every snapshot shows bcp_rows=800 even during healthy 1M-row loads
        # (log evidence: ACCT Bronze, 6 snapshots all showing bcp_rows=800,
        # load completed successfully with 1,085,431 rows).
        #
        # Fix: Use session-level cumulative row count from dm_exec_sessions
        # (snap.bcp_session_rows) which accumulates across all batches.
        # Fall back to request-level row_count if session rows unavailable.
        #
        # TODO-3 FIX: Exclude PREEMPTIVE_* wait types from stall candidacy.
        # Log evidence shows PREEMPTIVE_OLEDBOPS (seq 3) during healthy BCP —
        # this is BCP reading from the client pipe (legitimate I/O, not a stall).
        # PREEMPTIVE waits set status="suspended" but represent active work.
        #
        # TODO-4 FIX: Use log_used_pct delta as secondary progress indicator.
        # Even when row count appears frozen, rising log usage proves BCP is
        # writing. Only declare stall when BOTH row count AND log usage are flat.

        # Track log delta (regardless of stall evaluation)
        self._log_advancing = False
        if snap.log_used_pct is not None:
            if (self._last_log_used_pct is not None
                    and snap.log_used_pct > self._last_log_used_pct + 0.01):
                self._log_advancing = True
            self._last_log_used_pct = snap.log_used_pct

        # Use session-level cumulative rows if available, else request-level
        effective_row_count = getattr(snap, "bcp_session_rows", None) or snap.bcp_row_count

        # Evaluate stall: session suspended, NOT a preemptive wait, rows not advancing
        is_suspended = snap.bcp_status == "suspended"
        is_preemptive = (snap.bcp_wait_type or "").startswith("PREEMPTIVE_")

        if effective_row_count is not None and is_suspended and not is_preemptive:
            if (self._last_row_count is not None
                    and effective_row_count == self._last_row_count
                    and not self._log_advancing):
                if self._row_stall_since is None:
                    self._row_stall_since = elapsed
                elif (elapsed - self._row_stall_since) > self.abort_threshold:
                    snap.hang_detected = (
                        f"§5 ROW STALL — BCP session {snap.bcp_session_id} "
                        f"cumulative row_count has not advanced from "
                        f"{effective_row_count} for "
                        f"{elapsed - self._row_stall_since:.0f}s. "
                        f"Log usage also flat at {snap.log_used_pct:.1f}%. "
                        f"Current wait: {snap.bcp_wait_type} "
                        f"({snap.bcp_wait_time_ms}ms)."
                    )
                    return
            else:
        ...
        # Always update last row count (even outside stall evaluation)
        if effective_row_count is not None:
            self._last_row_count = effective_row_count
...
def _run_bcp_with_monitor(
    ...
    """
    # TODO-1: Skip monitor for loads that will complete before the first poll.
    skip_monitor = (
        expected_row_count is not None
        and expected_row_count < config.BCP_MONITOR_ROW_THRESHOLD
    )
    ...
    if skip_monitor:
        logger.debug(
            "BCP-MONITOR: Skipping monitor for %s — expected_row_count=%d "
            "below threshold %d",
            full_table_name, expected_row_count, config.BCP_MONITOR_ROW_THRESHOLD,
        )
    else:
        monitor.start()
    ...
    try:
        ...
        monitor.request_stop()
        monitor.safe_join(timeout=5)      # safe even if never started
...
def bcp_load(
    ...
    *,
    ...
    Throughput optimizations:
      - TDS packet size: -a 16384 (clamped to SSL/TLS ceiling)
    ...
    NOTE: The `atomic` parameter is retained for API compatibility but
    -b batch size is now ALWAYS included to prevent unbounded transactions.
    The batch size is determined by `is_stage` (100K for stage heaps,
    800 for Bronze clustered tables).

    All other behavior (hang monitor, retry, orphan cleanup) is unchanged.
    """
    ...
    for attempt in range(1, config.BCP_MAX_RETRIES + 1):
        ...
        logger.debug("BCP command: %s", " ".join(
            x if x != config.SQL_SERVER_PASSWORD else "[REDACTED]" for x in cmd
        ))
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                # FIX: Include both stdout and stderr in error message.
                # On Linux mssql-tools18, BCP frequently writes error details
                # to stdout (not stderr). Without stdout, errors show as
                # "stderr=" with no diagnostic information.
                stderr_msg = (result.stderr or "").strip()[:2000]
                stdout_msg = (result.stdout or "").strip()[:2000]
                raise BCPLoadError(
...
def _build_bcp_command(
    ...
    *,
    ...
    Fixes applied:
      - Packet size clamped to SSL/TLS ceiling (16384)
      - Field terminator uses hex 0x09 (unambiguous on Linux)
      - -k flag added (empty fields → SQL NULL)
      - -P flag added (explicit password, not relying on SQLCMDPASSWORD)
      - -b batch size always included (prevents unbounded single-transaction loads)
      - -c omitted when -f format file is provided (they conflict on Linux BCP)
      - -q flag added (QUOTED_IDENTIFIER ON — required for tables with filtered
        indexes, indexed views, computed column indexes, or XML type methods.
        Bronze tables have a filtered unique index on WHERE UdmActiveFlag=1;
        without -q, BCP fails with NativeError=1934 SET options error.)
    """
    # Clamp packet size to SSL/TLS ceiling
    packet_size = min(config.BCP_PACKET_SIZE, config.BCP_SSL_MAX_PACKET_SIZE)
    ...
    # -c (character mode) and -f (format file) conflict on Linux mssql-tools18.
    # BCP warns "-f overrides -c" and then may fail to parse the format file.
    # Only include -c when no format file is specified.
    if format_file:
        cmd.extend(["-f", format_file])
        logger.debug("W-13: Using BCP format file: %s (omitting -c to avoid conflict)", format_file)
    else:
        cmd.append("-c")

    cmd.extend([
        "-t", "0x09",                    # FIX: hex tab terminator (was "\\t")
        "-r", config.CSV_ROW_TERMINATOR,
        "-S", f"{config.SQL_SERVER_HOST},{config.SQL_SERVER_PORT}",
        "-U", config.SQL_SERVER_USER,
        "-P", config.SQL_SERVER_PASSWORD,  # FIX: explicit password flag
        "-Yo",                             # TrustServerCertificate
        "-q",                              # FIX: QUOTED_IDENTIFIER ON — required
                                           # for filtered indexes on Bronze tables
        "-a", str(packet_size),            # FIX: clamped to SSL ceiling
        "-k",                              # FIX: keep nulls (empty → NULL)
        "-m", "10",                        # FIX: explicit max errors
    ])

    # FIX: Always include batch size to prevent unbounded single-transaction loads.
    if is_stage:
...
def _build_bcp_env() -> dict[str, str]:
    """Build environment for BCP subprocess.

    Password is now passed via -P flag directly. SQLCMDPASSWORD kept as
    fallback for compatibility but is no longer the primary mechanism.
    """
...
def _read_recovery_model(cursor, database: str) -> str:
    """Return ``recovery_model_desc`` ('FULL'/'BULK_LOGGED'/'SIMPLE') or 'UNKNOWN'."""
    cursor.execute(
...
def _should_swap_recovery(current_model: str) -> bool:
    """Decide whether the pipeline may swap recovery model on this database.

    Honors ``config.PIPELINE_MANAGE_RECOVERY_MODEL``:
      * "never"  — never swap, regardless of current model.
      * "auto"   — swap only when current model is NOT FULL. Production
                   databases stay in FULL for PITR commitments.
      * "always" — swap regardless of current model. Use only on hosts
                   where you've verified PITR isn't required.

    Returns True if the swap to BULK_LOGGED is permitted.
    """
    mode = config.PIPELINE_MANAGE_RECOVERY_MODEL
    if mode == "never":
        return False
    if mode == "always":
        return True
    # Default: "auto" — protect FULL databases (production rule).
    return current_model != "FULL"


@contextmanager
def bulk_load_recovery_context(database: str):
    """Set BULK_LOGGED recovery during load, restore the original model after.

    Honors ``config.PIPELINE_MANAGE_RECOVERY_MODEL`` (auto/never/always).
    With the default "auto" gate, databases currently in FULL recovery are
    left untouched — production stays in FULL for PITR. Other databases
    are swapped to BULK_LOGGED for the load window and restored to their
    original model on exit (not hardcoded FULL).

    Idempotent against already-BULK_LOGGED state: if a parent context set
    BULK_LOGGED, this is a no-op restore-side too.
    ...
    switched_recovery = False
    original_model = "UNKNOWN"
    try:
        cursor = conn.cursor()
        original_model = _read_recovery_model(cursor, database)

        if not _should_swap_recovery(original_model):
            logger.info(
                "Recovery model on %s is %s and gate=%s — skipping ALTER DATABASE.",
                database, original_model, config.PIPELINE_MANAGE_RECOVERY_MODEL,
            )
        elif original_model != "BULK_LOGGED":
            logger.info(
                "Setting %s to BULK_LOGGED recovery model (was %s)",
                database, original_model,
            )
            cursor.execute(
                f"ALTER DATABASE {quote_identifier(database)} SET RECOVERY BULK_LOGGED"
            )
            # Item-17: Verify the ALTER actually succeeded.
            verify_model = _read_recovery_model(cursor, database)
            if verify_model == "BULK_LOGGED":
                switched_recovery = True
            else:
                logger.warning(
                    ...
                    database, verify_model,
                )
        else:
            logger.debug(
                "Recovery model already BULK_LOGGED on %s — skipping ALTER DATABASE",
                database,
            )
        cursor.close()
    finally:
    ...
    try:
        ...
        # Only restore if WE switched it. If a parent context set BULK_LOGGED,
        # let the parent handle the restore.
        if switched_recovery:
            conn = get_connection(database)
            try:
                cursor = conn.cursor()
                logger.info(
                    "Restoring %s to %s recovery model",
                    database, original_model,
                )
                cursor.execute(
                    f"ALTER DATABASE {quote_identifier(database)} "
                    f"SET RECOVERY {original_model}"
                )
                verify_model = _read_recovery_model(cursor, database)
                if verify_model != original_model:
                    logger.warning(
                        "Item-17: Failed to restore %s to %s recovery model "
                        "(current: %s). Manual intervention may be needed.",
                        database, original_model, verify_model,
                    )
                cursor.close()

                logger.info(
                    "R-3: Restored %s to %s. Log chain gap during BULK_LOGGED "
                    "window accepted — PITR not required during pipeline loads.",
                    database, original_model,
                )
            except Exception:
            ...
            logger.debug(
                "Skipping recovery model restore for %s — not managed by this context",
                database,
            )
...
def bulk_load_stage_context(database: str, full_table_name: str):
    """Combined context: BULK_LOGGED (idempotent) + sp_tableoption TABLOCK.
    ...
    Recovery model switching is idempotent: if the pipeline-level context
    has already set BULK_LOGGED, this skips the ALTER DATABASE to avoid
    database-level exclusive lock contention between workers. sp_tableoption
    is always managed per-table (it's table-scoped, not database-scoped).
    ...
    try:
        ...
        # 1. Recovery model swap — gated by PIPELINE_MANAGE_RECOVERY_MODEL.
        if not _should_swap_recovery(original_model):
            logger.info(
                "Recovery model on %s is %s and gate=%s — skipping ALTER DATABASE.",
                database, original_model, config.PIPELINE_MANAGE_RECOVERY_MODEL,
        ...
        # 2. Enable TABLOCK via sp_tableoption — table-scoped, always applies
        # regardless of recovery-model gate.
        logger.info(
    ...
    try:
        ...
        try:
            cursor = conn.cursor()
            # Always disable sp_tableoption (table-scoped, per-table is correct)
            logger.info(
            ...
            # Only restore recovery model if WE switched it — and restore to
            # the ORIGINAL model (could be SIMPLE on dev), not hardcoded FULL.
            if switched_recovery:
                logger.info(
                    "Restoring %s to %s recovery model", database, original_model,
                )
                cursor.execute(
                    f"ALTER DATABASE {quote_identifier(database)} "
                    f"SET RECOVERY {original_model}"
                ...
                logger.info(
                    "R-3: Restored %s to %s + disabled TABLOCK on %s",
                    database, original_model, full_table_name,
                )
            else:
                logger.debug(
                    "Skipping recovery model restore for %s — not managed by this context. "
                    "Disabled TABLOCK on %s.",
...
def bulk_load_bronze_context(database: str, full_table_name: str):
    """Combined context: BULK_LOGGED (idempotent) + LOCK_ESCALATION=DISABLE for Bronze.
    ...
    Recovery model switching is idempotent: if the pipeline-level context
    (main_small_tables.py / main_large_tables.py) has already set BULK_LOGGED,
    this context skips the ALTER DATABASE to avoid database-level exclusive
    lock contention between workers. It only restores to FULL if it was the
    one that switched it.
    ...
    try:
        ...
        # 1. Recovery model swap — gated by PIPELINE_MANAGE_RECOVERY_MODEL.
        # Default "auto" mode skips the swap when database is currently FULL,
        # protecting production. Otherwise swap to BULK_LOGGED for the load.
        if not _should_swap_recovery(original_model):
            logger.info(
                "Recovery model on %s is %s and gate=%s — skipping ALTER DATABASE.",
                database, original_model, config.PIPELINE_MANAGE_RECOVERY_MODEL,
            ...
            logger.debug(
                "Recovery model already BULK_LOGGED on %s — skipping ALTER DATABASE "
                "(pipeline-level context is active)",
                database,
            )

        # 2. Disable lock escalation for the Bronze table — table-scoped, always applies.
        logger.info(
            "Setting LOCK_ESCALATION=DISABLE on %s", full_table_name,
        )
        cursor.execute(
            f"ALTER TABLE {schema_table_quoted} SET (LOCK_ESCALATION = DISABLE)"
        )
        cursor.close()
    finally:
        conn.close()
    ...
    try:
        ...
        try:
            ...
            logger.info(
                "Restoring LOCK_ESCALATION=TABLE on %s", full_table_name,
            )
            cursor.execute(
                f"ALTER TABLE {schema_table_quoted} SET (LOCK_ESCALATION = TABLE)"
            )

            # Only restore recovery model if WE were the ones that switched it —
            # and restore to the ORIGINAL model (could be SIMPLE), not hardcoded FULL.
            if switched_recovery:
                logger.info(
                    "Restoring %s to %s recovery model", database, original_model,
                )
                ...
                logger.info(
                    "R-3: Restored %s to %s + LOCK_ESCALATION=TABLE on %s",
                    database, original_model, full_table_name,
                )
            else:
                logger.debug(
            ...
            logger.exception(
                "Failed to restore LOCK_ESCALATION / recovery model for %s", database
            )
        finally:
            conn.close()
...
@contextmanager
def bulk_load_bronze_first_run_context(database: str, full_table_name: str):
    """Combined context: BULK_LOGGED + sp_tableoption TABLOCK for first-run Bronze loads.

    BCP-HANG-FIX-v3: When a Bronze table is empty (first-run backfill), there
    are no concurrent readers. This makes it safe to use the old pipeline's
    TABLOCK approach, which solves the connection-drop hang by:

      1. TABLOCK acquires a Bulk Update (BU) lock at session start, keeping
         the SQL Server session active continuously. Without TABLOCK, the
         800-row micro-commits create idle gaps where the session has no
         active lock — firewalls and TLS keep-alive can kill the connection
         during these gaps.

      2. With TABLOCK + large batch size (100K), a 1.9M-row load requires
         ~19 batch commits instead of ~2,422 with 800-row batches. This is
         19x fewer log flushes, 19x fewer network round-trips, and 19x
         fewer opportunities for connection drops.

      3. TABLOCK enables minimal logging on the clustered index (SQL Server
         2019+ FastLoadContext), reducing transaction log I/O by 90-99%.

    This is functionally identical to bulk_load_stage_context but exists as
    a separate entry point for:
      - Clear intent: callers see "first_run" and understand the purpose
      - Future divergence: Bronze-specific first-run logic (e.g., ORDER hint
        for clustered key pre-sorting) can be added here without affecting
        Stage context
      - Logging: Bronze-specific log messages distinguish first-run loads
        from normal Stage loads in production monitoring

    IMPORTANT: Only use this when the Bronze table has 0 active rows. If the
    table has ANY active rows, use bulk_load_bronze_context instead to avoid
    blocking concurrent readers with an exclusive lock.

    Usage:
        if active_rows == 0:
            with bulk_load_bronze_first_run_context("UDM_Bronze", "UDM_Bronze.dna.TABLE_scd2_python"):
                bcp_load(..., is_stage=True)  # Uses 100K batch size
        else:
            with bulk_load_bronze_context("UDM_Bronze", "UDM_Bronze.dna.TABLE_scd2_python"):
                bcp_load(...)  # Uses 800 batch size
    """
    logger.info(
        "BCP-HANG-FIX-v3: Using first-run Bronze context (TABLOCK) for %s",
        full_table_name,
    )
    # Delegate to Stage context which handles BULK_LOGGED + sp_tableoption.
    # The sp_tableoption 'table lock on bulk load' works identically on
    # clustered-index tables (Bronze) and heaps (Stage) — it triggers BU
    # lock acquisition for all bulk import operations on the table.
    with bulk_load_stage_context(database, full_table_name):
        yield
...
def bcp_load_parallel(
    ...
    Inherits all BCP command fixes (packet size clamping, hex terminators,
    -k keep nulls, -P password, -b batch size).
    """
    ...
    # _split_csv_file hard-cleans chunk_dir internally before split.
    chunk_paths = _split_csv_file(csv_path, n_streams, chunk_dir)

    # Pre-flight chunk validation: count lines in each chunk and verify the
    # sum equals the input CSV's line count. Catches split mishaps (stale
    # leftover files, partial writes, wc-l/split-l line-end mismatches)
    # BEFORE submitting any BCP jobs. The earlier we detect a 2x-style
    # discrepancy, the easier it is to surface — rather than chasing a
    # phantom mid-load row count mismatch that times out at 7200s.
    chunk_line_counts: dict[str, int] = {}
    chunk_total = 0
    for cp in chunk_paths:
        n = _count_lines(str(cp))
        chunk_line_counts[str(cp)] = n
        chunk_total += n
    if chunk_total != total_lines:
        raise BCPLoadError(
            f"Parallel BCP split mismatch for {full_table_name}: "
            f"input CSV has {total_lines} lines but split produced "
            f"{len(chunk_paths)} chunks summing to {chunk_total} lines. "
            f"Chunk line counts: {chunk_line_counts}. "
            f"Possible stale chunks in {chunk_dir} or wc/split disagreement on line endings."
        )
    ...
    logger.info(
        ...
        sorted(chunk_line_counts.values(), reverse=True)[:10],
    )

    bcp_env = _build_bcp_env()
    total_rows = 0
    errors = []
    chunk_results: dict[str, int] = {}
    ...
    def _load_chunk(chunk_path: str) -> tuple[str, int]:
        ...
        # Redact password in debug logging
        safe_cmd = [x if x != config.SQL_SERVER_PASSWORD else "[REDACTED]" for x in cmd]
        logger.debug("Parallel BCP chunk command: %s", " ".join(safe_cmd))
        ...
        if result.returncode != 0:
            # FIX: Include both stdout and stderr — Linux BCP writes errors to stdout
            stderr_msg = (result.stderr or "").strip()[:1000]
            stdout_msg = (result.stdout or "").strip()[:1000]
            raise BCPLoadError(
                f"Parallel BCP chunk failed (rc={result.returncode}): "
                f"{chunk_path}: stderr={stderr_msg} | stdout={stdout_msg}"
            )
        rows = _parse_rows_copied(result.stdout)

        # Per-chunk validation: BCP rows-copied must match chunk file line
        # count. A silent partial load (rc=0 but rows < lines) would
        # otherwise sum into a final mismatch error 7200s later.
        expected_chunk_rows = chunk_line_counts.get(chunk_path, -1)
        if expected_chunk_rows >= 0 and rows != expected_chunk_rows:
            raise BCPLoadError(
                f"Parallel BCP per-chunk count mismatch on {chunk_path}: "
                f"file has {expected_chunk_rows} lines, BCP reported "
                f"{rows} rows copied. BCP stdout: {result.stdout[:500]}"
            )
        ...
        with ThreadPoolExecutor(max_workers=n_streams) as executor:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    total_rows += rows
                    chunk_results[chunk_file] = rows
                    logger.debug(
                        "Parallel BCP: Chunk %s loaded %d rows (file had %d lines)",
                        chunk_file, rows, chunk_line_counts.get(chunk_file, -1),
                    )
                except Exception as e:
    ...
    if expected_row_count is not None and total_rows != expected_row_count:
        raise BCPLoadError(
            ...
            f"Chunk results: {chunk_results}. "
            f"Pre-load chunk line counts: {chunk_line_counts}."
        )
...
def _split_csv_file(filepath: str, n_chunks: int, output_dir: Path) -> list[Path]:
    ...
    Hard-cleans ``output_dir`` before splitting so stale chunks from a
    prior run that crashed before reaching the ``shutil.rmtree(chunk_dir)``
    cleanup can't pollute the current run's glob. Without this, a previous
    run with N+k streams could leave chunk_008..chunk_015 behind, and the
    current run's glob picks up all 16, double-loading or partially-loading
    the table.
    """
    # Force-clean the chunk dir so glob() sees only this run's output.
...
def smart_load(
    ...
    *,
    ...
    Decision matrix:
      - < 1,000 rows AND df provided: pyodbc fast_executemany
      - >= 1M rows AND Stage table: parallel BCP streams
      - Everything else: single BCP stream

    Batch size is always included (-b flag) regardless of atomic setting.
    """
...
def _safe_delete_csv(csv_path: Path | str) -> None:
    """MEM-1: Delete CSV file immediately after successful BCP load.

    Frees disk space (or tmpfs RAM if CSV_TMPFS_ENABLED) as soon as the
    file has served its purpose as a BCP transport. Errors are logged
    but non-fatal — the end-of-pipeline cleanup_csvs sweep is the safety net.
    """
    try:
        p = Path(csv_path)
        if p.exists():
            p.unlink()
            logger.debug("MEM-1: Deleted CSV after BCP load: %s", p.name)
    except Exception:
        logger.debug("MEM-1: Could not delete %s — cleanup_csvs will catch it", csv_path)
