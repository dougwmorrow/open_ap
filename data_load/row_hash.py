...
def _normalize_for_hashing(
    ...
    R-10.2: ``exclude_cols`` drops columns from the hash input so high-churn
    metadata columns (typical DNA: ``DATELASTMAINT``) don't generate phantom
    CDC updates. Columns are still extracted and written to Stage/Bronze
    normally — only excluded from the change-detection hash. Mirrors the
    legacy ``@ExcludeColumnsFromHash`` proc parameter.

    Args:
        df: DataFrame with source columns.
        source_is_oracle: If True, normalize empty strings to NULL (E-1).
        exclude_cols: Column names to drop from the hash input. Matched
            case-sensitively against ``df.columns``. Missing names log a
            WARNING and are ignored (likely config typo).
    ...
    if exclude_cols:
        ...
        unknown = exclude_set - df_col_set
        if unknown:
            logger.warning(
                "R-10.2: ExcludeFromHash references %d column(s) not present "
                "in DataFrame: %s. Check UdmTablesList.ExcludeFromHash spelling "
                "(match is case-sensitive). Ignoring unknown names.",
                len(unknown), sorted(unknown),
            )
        applied = [c for c in source_cols if c in exclude_set]
        if applied:
            logger.info(
                "R-10.2: Excluding %d column(s) from hash: %s",
                len(applied), applied,
            )
            source_cols = [c for c in source_cols if c not in exclude_set]
...
def add_row_hash(
    ...
    (V-1, V-2, E-1, E-4, W-2, W-3, E-20, R-10.2).

    Args:
        df: DataFrame with source columns.
        source_is_oracle: If True, normalize empty strings to NULL (E-1).
        exclude_cols: R-10.2 — column names dropped from hash input (e.g.
            ``DATELASTMAINT``) to prevent phantom CDC updates on rolling
            metadata timestamps.
    ...
    """
    # W-9 STATUS — polars-hash 0.5.6 in production:
    #   The pipeline now pins polars==1.38.1 + polars-hash==0.5.6 +
    #   polars-runtime-32==1.38.1 (see requirements.txt). Hash output is
    #   stable on this stack. Future upgrades carry H-2 risk (hash format
    #   change → mass CDC update wave on every table). Validate hash
    #   stability against a reference dataset before any version bump.
    import polars_hash  # noqa: F401 — registers .chash namespace

    df, hash_exprs = _normalize_for_hashing(df, source_is_oracle, exclude_cols)
...
def add_row_hash_fallback(
    ...
    Args:
        df: DataFrame with source columns.
        source_is_oracle: If True, normalize empty strings to NULL (E-1).
        exclude_cols: R-10.2 — same semantics as ``add_row_hash``.
