...
def normalize_boolean_to_int8(df: pl.DataFrame) -> pl.DataFrame:
    """C-3b: Normalize all Boolean columns to Int8 immediately after extraction.

    ConnectorX reads SQL Server BIT columns as Boolean. When these DataFrames
    are later concatenated with CDC-annotated DataFrames (where _cdc_is_current
    is Int8), the Boolean/Int8 mismatch causes dtype widening in _safe_concat.

    Unlike cast_bit_columns() (which runs at BCP output time), this runs at
    extraction INPUT time — specifically after reading from Stage/Bronze via
    ConnectorX. This eliminates the mismatch at its source so downstream
    concat never sees Boolean columns from SQL Server BIT.

    Also handles _cdc_is_current which is stored as BIT in Stage DDL but
    should be Int8 in the Polars pipeline (matching CDC_SCHEMA).
    """
    bool_cols = [
        col for col, dtype in zip(df.columns, df.dtypes)
        if dtype == pl.Boolean
    ...
    logger.debug(
        "C-3b: Normalizing %d Boolean columns to Int8: %s",
...
def reorder_columns_for_bcp(
    ...
    fill_null_columns: set[str] | None = None,
) -> pl.DataFrame:
    ...
    Args:
        ...
        fill_null_columns: SCD-1b: Target columns that may be missing from the DataFrame
            because they were excluded from extraction (e.g. RowGuidCol columns). If these
            columns exist in the target but not in the DataFrame, they are added as NULL
            instead of raising ColumnOrderError. BCP loads NULLs into these positions,
            preserving existing data in those columns.

    Returns:
        DataFrame with columns reordered to match target.

    Raises:
        ColumnOrderError: If DataFrame has columns not in target or is missing columns
                          that are not in fill_null_columns.
    """
    from data_load.schema_utils import get_target_column_order

    if fill_null_columns is None:
        fill_null_columns = set()
    ...
    if missing_in_df:
        # SCD-1b: Columns excluded from extraction but still in target —
        # pad with NULL so BCP positional mapping stays correct.
        fillable = missing_in_df & fill_null_columns
        unfillable = missing_in_df - fill_null_columns

        if unfillable:
            raise ColumnOrderError(
                f"DataFrame is missing columns expected by {full_table_name}: {unfillable}"
            )

        if fillable:
            logger.info(
                "SCD-1b: Adding %d NULL column(s) to match target %s "
                "(excluded from extraction): %s",
                len(fillable), full_table_name, sorted(fillable),
            )
            df = df.with_columns(
