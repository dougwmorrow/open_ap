...
**For fresh Claude session**: read this file first, then the immutable snapshot at `docs/migration/_session_snapshots/2026-05-19-7a810b9.md` (§4 Deeper insights = irreplaceable architectural-decision context), then `docs/migration/INDEX.md` → `CURRENT_STATE.md` → `HANDOFF.md` → `CLAUDE.md` per CCL Stage 0+1 discipline.
...
- **pytest baseline (this chat's D125 module scope)**: ~119 Tier 0/1 tests pass across D125 implementation modules at HEAD `6868564`:
  ...
  - `tests/tier1/test_orchestrator_cdc_mode_dispatch.py` — 18+1 tests (B-544 v1 orchestrator dispatch)
...
- **Cumulative session delta (D125 / parquet→SCD2 scope only; meta-discipline chat's work is separate)**:
  ...
  - **15 B-Ns CLOSED** this arc: B-345 + B-334 + B-498 + B-337 + B-538 + B-541 + B-343 + B-542 + B-543 + B-544 v1 + B-545 v1 + B-546 + B-553 + B-554 + B-547
  - **2 B-Ns inline-closed** this arc: B-548 + B-549
  - **B-Ns OPEN from this arc** (next-step queue): B-552 + B-555 + B-556 + B-557 + B-560 + B-561 + P-24
  ...
  - **NEW production code** (extensions): `orchestration/table_config.py` (B-543; +18 LOC TableConfig.cdc_mode field + loader); `orchestration/pipeline_steps.py` (B-544 v1; +82 LOC dispatch helpers); `orchestration/large_tables.py` + `orchestration/small_tables.py` (B-544 v1; ~30 LOC dispatch wiring each); `scd2/engine.py` (R1.3; B-498 + B-334; +106 LOC source_verifier_fn parameter + helper); `utils/idempotency_ledger.py` (R1.8; B-337; +30 LOC D119 forensic-preservation 3-site defense); `orchestration/table_lock.py` (R1.7; B-345; constant promotion)
  - **NEW EventType family registrations**: CLI_FLIP_CDC_MODE (25th) + CLI_VALIDATE_PARQUET_VS_STAGE (26th) + MIGRATION_CDC_MODE_COLUMN + PARQUET_WRITE
...
2. **`docs/migration/_session_snapshots/2026-05-19-7a810b9.md`** — immutable snapshot at session-pause point; §4 Deeper insights captures 6 architectural decisions + 6 cross-cohort patterns + 3 convergence-discipline events + 6 meta-discipline observations + reviewer-finding context table. **IRREPLACEABLE substrate for D125 architectural-decision context**.
3. **`docs/migration/UDM_PIPELINE_CDC_MODE_3WAY_DISPATCH_PLAN_2026-05-19.md`** — D125 canonical plan; §2.3 transition matrix + §5.2 dispatch pseudocode + §8.2/§8.3 R1/R2 checklists (post-B-547 closure most items checked off)
4. **`docs/migration/BACKLOG.md`** L1095+ (B-552 + B-555 entries) + L1106+ (B-560 + B-561 + B-557 entries)
...
7. **`CLAUDE.md`** L209+ CLI_* registry (now 27 tools post-B-562 Component A) + Do-NOT rules section (LIFTED B-545 rule + active BOTH-mode Parquet-before-CDC rule)
...
- **B-552** (HIGH; WSJF 3.5): **v2 of B-544 — `cdc_mode='parquet_snapshot'` end-to-end Parquet→replay→SCD2 path**. HARD-PREREQUISITE for RB-16 Step 2 operational meaning — orchestrator currently raises NotImplementedError for `cdc_mode == 'parquet_snapshot'` mode. Scope: bypass `run_cdc_promotion()` entirely; call `replay_parquet_snapshot()` to materialize df_current; construct cdc_result-compatible structure for `run_scd2_promotion()` (requires either adapter OR `run_scd2_promotion` refactor); handle delete-detection (legacy CDC computes via hash anti-join on Stage; replay path needs day-N vs day-N-1 Parquet diff OR equivalent). Estimated ~150-200 LOC orchestrator + adapter + Tier 1 tests. Closure target: Phase 2 R2.

### MEDIUM priority — interpretation-gap closures
...
- **B-556** (LOW; WSJF 2.0): **Apply-path Tier 0/1 tests for `tools/flip_cdc_mode.py` + `tools/validate_parquet_vs_stage.py`**. Currently only dry-run paths are tested; non-dry-run UPDATE/INSERT/commit/rollback paths mechanically uncovered. Closure target: opportunistic alongside B-552/B-555 OR next CLI-tool authoring.
...
- **P-24** (LOW; cosmetic): **Document LIFTED-Do-NOT-rule format in CLAUDE.md Do-NOT-rules section header**. First instance precedent set at `00039a1`; format = `~~Do NOT ...~~` strikethrough body + **LIFTED YYYY-MM-DD** annotation + B-N closure citations + empirical-anchor preservation. Closure target: opportunistic at next Do-NOT rule lift OR round close-out.

### Recommended next-step

**B-552** is the highest-impact next deliverable — closes the LAST D125 production-cutover blocker. Substantive scope (~150-200 LOC) but unblocks RB-16 Step 2 + removes NotImplementedError from orchestrator. Alternative: **B-547 inventory polish** at `05_RUNBOOKS.md` L19-20 (RB-N entries not strictly monotonic post-RB-18 + RB-16 additions; POLISH_QUEUE candidate; LOW effort).

## This session's commit chain (most-recent 10 D125-arc commits; older arc commits in snapshot §5)
...
e94d136  build(round-6): B-545 v1 partial closure — validate_parquet_vs_stage.py parity-check CLI
0ad5bcc  build(round-6): B-546 closure — flip_cdc_mode.py operator CLI per D63 + D125 + RB-16
```

(Earlier arc commits — R1.3 + R1.7 + R1.8 cohort + B-344 adoption + D125 plan + B-542 + B-543 + B-544 v1 + B-343 RB-18 — enumerated in snapshot §5 with significance annotations.)
...
- D125 / scd2 chat owns: `migrations/cdc_mode_column.py`, `tools/flip_cdc_mode.py`, `tools/validate_parquet_vs_stage.py`, `orchestration/*`, `scd2/*`, `utils/idempotency_ledger.py`, RB-16 + RB-18 procedures, D125 plan + entry
- meta-discipline chat owns: `udm-*` skills, `tools/pre_commit_checks.py`, `tools/claim_next_bn.py`, B-558 Phase 2.1 substrates
- Shared / coordination-required: `CLAUDE.md` (Do-NOT rules + CLI_* registry + L407 substrate enumeration), `docs/migration/BACKLOG.md`, `docs/migration/_validation_log.md`, `docs/migration/03_DECISIONS.md`
...
**Session arc theme**: started 2026-05-18 with operator question "I am testing the parquet data load process for large tables. How do I set up my env files and General.dbo.UDMTablesList to ensure that the AuditLog table from CCM loads parquet files properly into VendorFiles network drive?" → arc covered R1 cohort SCD2 foundation (R1.3+R1.7+R1.8) + B-344 RB-15 adoption + D125 3-mode CDC dispatch plan + 7 D125 implementation B-N closures + B-547 RB-16 procedure rewrite + 6 cross-cohort gap-checks.

**Operator workflow now end-to-end complete** for D125 cutover:
...
# 5. After ≥30-day clean parity period, canonical cutover (REQUIRES B-552 closure first):
python3 tools/flip_cdc_mode.py --apply \
...
Steps 1-4 are PRODUCTION-READY at HEAD `6868564`. Step 5 requires B-552 closure first to remove the orchestrator's NotImplementedError for `cdc_mode == 'parquet_snapshot'`. B-555 (v2 per-PK hash) recommended before Step 5 for definitive parity interpretation.

---

*Authored 2026-05-19 per B-562 Component B Phase 1 SESSION_RESUME/active/ directory convention. State-pointer is point-in-time (commit `6868564`); refresh at next D125-arc substantive commit. Canonical deep-context substrate: `docs/migration/_session_snapshots/2026-05-19-7a810b9.md`.*
