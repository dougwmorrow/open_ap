...
4. **Apply `udm-progress-logger`** per CLAUDE.md hard rule 9 -- update canonical trackers + RELATED markdown files based on build type. Per user-direction 2026-05-15 ("Update the skill so it includes updating markdown files related to the build that just took place"), the cascade now walks a per-build-type checklist. **NOTE — `udm-progress-logger` Step 0 (post-compaction tracker re-Read) is MANDATORY at turn start before any Edit on large trackers**; the cascade inherits this discipline. See `.claude/skills/udm-progress-logger/SKILL.md` Step 0 for the fresh-Read + post-Edit-Grep-verification procedure. Empirical anchor: commit `db77516` 2026-05-16 (stale-Edit-state caused temporary CURRENT_STATE↔BACKLOG inconsistency).
   ...
   **Always update (5 canonical trackers — universal regardless of build type)**:
   ...
   - `docs/migration/HANDOFF.md` `## §14. Last updated` section narrative prepended — mirror of CURRENT_STATE for fresh-agent onboarding. The section IS active (commit `570ac67` 2026-05-16 is the canonical-application example: prepend a new dated parenthetical entry matching the CURRENT_STATE L7 format; demote prior most-recent entry to an "Earlier <date>:" line below). SKIP-with-justification when: edit is doc-only metadata polish OR single-tracker P-N closure OR already-mirrored from a prior commit in the same session — explicit "UNTOUCHED-AS-EXPECTED (reason)" required per the verification procedure below; "no §14 anchor exists" is NOT a valid justification (the anchor exists at HANDOFF L425).
