...
| EventType constant | (a) CLAUDE.md L207 CLI_\* family registry updated (if CLI_\* type; corrected from stale L325 anchor per B-331 multi-site fix discipline 2026-05-17); (b) `phase1/02_configuration.md` family registry updated |
...
**Empirical anchor**: commit `63edcbc` (B-331 opening) REVIEW section cited `"SUBSTANTIVE classification"` alone without ≤50 LOC + no-new-public-surface + no-SUBSTRATE_EDIT citations. 2nd-pass design reviewer agentId `a6b24c207dd9fdb75` Q5 finding flagged the precedent-drift risk: future producers may misread `"any SUBSTANTIVE permits inline self-review"` and apply to 200+ LOC commits. This is the **1st-event anchor for sub-class promotion in HANDOFF §8 Pitfall list** (4 more events needed per the 5-event convention). **Clarification per multi-agent reviewer agentId `ac668d97b36aef385` IMPROVE #2**: the directive ITSELF (this section's hard rule) is **binding NOW** as of v1.2.0 — the 5-event convention applies only to PROMOTION of the pattern into the canonical HANDOFF §8 Pitfall #9 sub-class list (a documentation taxonomy event), NOT to the directive's binding force. Future producers MUST comply with v1.2.0's REVIEW citation discipline regardless of sub-class promotion status.
...
- **v1.2.0** (2026-05-17) — per B-331 opening + 2nd-pass design reviewer agentId `a6b24c207dd9fdb75` Q5 finding on commit `63edcbc` + 3rd-pass multi-agent reviewer agentId `ac668d97b36aef385` IMPROVE #1+#2 inline-fixes. **MINOR (directive addition) per D98 semver discipline**: added explicit inline-self-review citation discipline at REVIEW section template — requires verbatim citation of `≤50 LOC + no-new-public-surface + no-SUBSTRATE_EDIT` carve-out conditions when claiming inline self-review (SUBSTANTIVE classification alone no longer sufficient). Added anti-pattern enumeration distinguishing ❌ omission patterns from ✅ compliant format. Also corrects stale L325 anchor at Step 1 TEST table (L50) to L207 per B-331 multi-site fix discipline (IMPROVE #1 — Pitfall #9.k micro-instance fixed inline). Clarified "1st-event anchor for sub-class formalization" framing (L158) — directive is binding NOW per the hard rule at v1.2.0; sub-class promotion in HANDOFF §8 Pitfall list (a documentation taxonomy event) requires 4 more events (IMPROVE #2 — prevents misreading directive as optional until 5 events accumulate). Empirical anchor: commit `63edcbc` REVIEW section cited "SUBSTANTIVE classification" alone → 2nd-pass reviewer flagged precedent-drift risk (future producers may misread "any SUBSTANTIVE permits inline self-review" and apply to 200+ LOC commits).
