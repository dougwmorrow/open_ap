...
# E-11/P0-2: Pipeline-internal columns added by CDC/SCD2 logic.
# These exist in Stage/Bronze tables (and UdmTablesColumnsList) but are NOT
# source columns — they must be excluded from schema validation and evolution
# comparisons. Shared between validate_source_schema() and _evolve_table().
_INTERNAL_COLUMNS = {
    ...
    # SCD2 Phase 1: R-1/R-3 source-date pair. These are added by
    # migrations/scd2_phase1_config.py on existing Bronze tables and by
    # schema/table_creator.py for newly-created Bronze tables. They must
    # be excluded from source-vs-Bronze comparison or every pipeline run
    # will falsely log them as "removed from source" (SCD2-P1-a).
...
def validate_source_schema(table_config: TableConfig, df: pl.DataFrame) -> list[str]:
    ...
    # Only compare source columns (exclude internal pipeline columns)
    df_cols = {c for c in df.columns if not c.startswith("_")}
    expected_cols = {
        c for c in stage_cols
        if c not in _INTERNAL_COLUMNS and c not in table_config.exclude_columns
    }
...
def _evolve_table(
    ...
    existing_source_cols = {
        ...
        if m.column_name not in _INTERNAL_COLUMNS
    }
    ...
    for col_name in common:
        # Skip Null-typed columns — all values are NULL, so Polars has no
        # type information. This happens on sparse days with very few rows.
        # The existing Stage type is authoritative; NULL values are type-agnostic.
        if source_cols[col_name] == pl.Null:
            logger.debug(
                "Skipping type check for %s — Polars dtype is Null (all values NULL in extraction)",
                col_name,
            )
            continue
...
def _types_compatible(existing: str, new: str) -> bool:
    ...
    E-7b: ConnectorX maps SQL Server DECIMAL/NUMERIC/MONEY to Polars Float64
    (-> FLOAT in SQL), while pyodbc maps them to Polars Decimal(p,s)
    (-> DECIMAL(p,s) in SQL). Both are valid numeric representations of the
    same source column. Without this compatibility, a ConnectorX-to-pyodbc
    fallback swap causes a false SchemaEvolutionError on the next run.
    """
    ...
    compatible_pairs = {
        # Integer widening (ConnectorX dtype drift)
        ("INT", "BIGINT"),
        ("BIGINT", "INT"),
        ("SMALLINT", "INT"),
        ("SMALLINT", "BIGINT"),
        ...
        ("REAL", "FLOAT"),
        ("FLOAT", "REAL"),
        # E-7b: FLOAT <-> DECIMAL — ConnectorX returns Float64 for DECIMAL/
        # NUMERIC/MONEY columns, pyodbc returns Decimal(p,s). Both are valid
        # numeric mappings of the same source column.
        ("FLOAT", "DECIMAL"),
        ("DECIMAL", "FLOAT"),
        ("FLOAT", "NUMERIC"),
        ("NUMERIC", "FLOAT"),
        # DECIMAL <-> NUMERIC are aliases in SQL Server
        ("DECIMAL", "NUMERIC"),
        ("NUMERIC", "DECIMAL"),
        # MONEY types — pyodbc may return these as Decimal too
        ("FLOAT", "MONEY"),
        ("MONEY", "FLOAT"),
        ("DECIMAL", "MONEY"),
        ("MONEY", "DECIMAL"),
