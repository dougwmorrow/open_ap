...
def _pyodbc_connection_string(database: str) -> str:
    return (
        ...
        f"SERVER={configuration.SQL_SERVER_HOST},{configuration.SQL_SERVER_PORT};"
        f"DATABASE={database};"
        f"UID={configuration.SQL_SERVER_USER};"
...
def _connectorx_uri(database: str) -> str:
    usr = quote_plus(configuration.SQL_SERVER_USER)
    pwd = quote_plus(configuration.SQL_SERVER_PASSWORD)
    return (
        f"mssql://{usr}:{pwd}@{configuration.SQL_SERVER_HOST}:{configuration.SQL_SERVER_PORT}"
...
def get_silver_connection() -> pyodbc.Connection:
    return get_connection(configuration.SILVER_DB)


def get_gold_connection() -> pyodbc.Connection:
    return get_connection(configuration.GOLD_DB)


def silver_connectorx_uri() -> str:
    return _connectorx_uri(configuration.SILVER_DB)


def gold_connectorx_uri() -> str:
    return _connectorx_uri(configuration.GOLD_DB)

def get_stage_connection() -> pyodbc.Connection:
    return get_connection(configuration.STAGE_DB)


def get_bronze_connection() -> pyodbc.Connection:
    return get_connection(configuration.BRONZE_DB)


def get_general_connection() -> pyodbc.Connection:
    return get_connection(configuration.GENERAL_DB)


def get_source_connection(host: str, database: str, port: int = 1433) -> pyodbc.Connection:
    """Create a pyodbc connection to a source system server.

    Unlike get_connection() which always targets the UDM server
    (config.SQL_SERVER_HOST), this connects to an arbitrary source host
    using the host/database values from General.dbo.UdmTablesList.

    Uses the same SQL auth credentials as the UDM connections — if source
    systems require different auth, extend UdmTablesList with per-source
    credentials or use Trusted_Connection for domain-joined servers.

    Args:
        host: Source server hostname (e.g. 'PDCAAGDNA02').
        database: Source database name (e.g. 'CCMREPORT').
        port: Source server port (default 1433).

    Returns:
        A pyodbc connection with autocommit=True.
    """
...
def stage_connectorx_uri() -> str:
    return _connectorx_uri(configuration.STAGE_DB)

def bronze_connectorx_uri() -> str:
    return _connectorx_uri(configuration.BRONZE_DB)

def silver_connectorx_uri() -> str:
    """ConnectorX URI for UDM_Silver reads."""
    return _connectorx_uri(configuration.SILVER_DB)
...
def general_connectorx_uri() -> str:
    return _connectorx_uri(configuration.GENERAL_DB)
...
def verify_rcsi_enabled() -> None:
    ...
    """
    try:
        with cursor_for(configuration.BRONZE_DB) as cur:
            cur.execute(
                "SELECT is_read_committed_snapshot_on "
                "FROM sys.databases WHERE name = ?",
                configuration.BRONZE_DB,
            )
            row = cur.fetchone()
            ...
            if row is None:
                ...
                logger.warning(
                    ...
                    configuration.BRONZE_DB, configuration.SCD2_UPDATE_BATCH_SIZE,
                    configuration.BRONZE_DB,
                )
            else:
                logger.info("E-9: RCSI verified enabled on %s", configuration.BRONZE_DB)
    except Exception:
        logger.warning(
            "E-9: Could not verify RCSI on %s — continuing without verification",
            configuration.BRONZE_DB, exc_info=True,
        )
